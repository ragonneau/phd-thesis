...
\begin{enumerate}
    ...
    \item \Cref{ch:sqp} presents an overview of the classical \gls{sqp} method and provides some perspectives.
    ...
    \item \Cref{ch:cobyqa-introduction} presents a new model-based \gls{dfo} method named \gls{cobyqa}.
    ...
    \item \Cref{ch:cobyqa-subproblems} introduces in details the methods employed by \gls{cobyqa} to solve its various subproblems.
    \item \Cref{ch:cobyqa-implementation} provides details on the Python implementation of \gls{cobyqa}.
    Such an implementation is crucial and very challenging, due to the nature of \gls{dfo}.
    However, we want to contribute an open-source, robust, and easily-readable package to the community.
    This chapter also presents some numerical experiments that compare \gls{cobyqa} with \gls{newuoa}, \gls{bobyqa}, \gls{lincoa}, and \gls{cobyla}.
\end{enumerate}
