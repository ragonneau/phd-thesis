...
\todo[noline]{Change the line spacing to have 130/150 pages}

In this chapter, we present the basic concepts of optimization that we will necessitate later in this thesis.
In particular, we introduce the notion of \gls{dfo}, which is the cornerstone of the research work presented here.
We provide several examples of \gls{dfo} problems to support the need for \gls{dfo} solvers in industrial, engineering, and research applications.
At the end of this introduction, we present two benchmarking tools, referred to as performance and data profiles.
These tools will be helpful throughout this thesis to compare and assess the performance of \gls{dfo} methods.

\section{Overview of \glsfmtlong{dfo}}
\label{sec:overview}

\index{\glsfmtshort{dfo}|(}Optimization is the study of extremal points and values of mathematical functions.
It aims at minimizing (or maximizing) a real-valued function~$\obj$\nomenclature[Fa]{$\obj$}{Real-valued objective function defined on~$\R^n$}, referred to as the \emph{objective function}, within a given set of points~$\fset \subseteq \R^n$\nomenclature[Nb]{$\subseteq$}{Set inclusion notation}\nomenclature[Sf]{$\R^n$}{Real coordinate space of dimension~$n$}\nomenclature[Sj]{$\fset$}{Feasible set, included in~$\R^n$}, referred to as the \emph{feasible set}.
...
\index{\glsfmtshort{dfo}!model-based!trust-region|(}Another well-known example of \gls{dfo} applications is the parameter tuning of nonlinear optimization methods~\cite{Audet_Orban_2006}.
...
The \gls{bfo}~\cite{Porcelli_Toint_2017}, a method for bound-constrained problems mixing continuous and discrete variables, is an example of self-tuned \gls{dfo} methods.\index{\glsfmtshort{dfo}!model-based!trust-region|)}
...
It has been included in GEMSEO~\cite{Gallard_Etal_2018}, an engine for \gls{mdo} initiated by a team from IRT Saint Exup{\'{e}}ry\footnote{See \url{https://www.irt-saintexupery.com}.} in France.\index{\glsfmtshort{dfo}|)}
...
\index{\glsfmtshort{dfo}|(}As summarized in~\cite{Conn_Scheinberg_Vicente_2009b}, two main strategies have been developed for solving \gls{dfo} problems.
...
\index{\glsfmtshort{dfo}!direct-search|(}An early example of \gls{dfo} is a method from \citeauthor{Fermi_Metropolis_1952}~\cite{Fermi_Metropolis_1952}, who developed in \citeyear{Fermi_Metropolis_1952} a nonlinear least-squares solver on MANIAC, an computer based on the von Neumann architecture.
...
This improves both the performance and the worst-case complexity bounds compared with traditional direct-search methods.\index{\glsfmtshort{dfo}!direct-search|)}

\subsection{Model-based methods}
\label{subsec:model-based-methods}

\index{\glsfmtshort{dfo}!model-based|(}Unlike direct-search methods, model-based methods approximate locally the functions involved in the optimization problems by simple functions called \emph{models} or \emph{surrogates}.
To make model-based methods globally convergent, the models are exploited by a globalization strategy, such as a \emph{line-search}\index{\glsfmtshort{dfo}!model-based!line-search} framework~\cite[Ch.~3]{Nocedal_Wright_2006} or a \emph{trust-region} framework~\cite{Conn_Gould_Toint_2000,Yuan_2015}.
...
The work of \citeauthor{Bandeira_Scheinberg_Vicente_2012} has motivated many investigations on methods that use randomized models, such as~\cite{Chen_Menickelly_Scheinberg_2018}.\index{\glsfmtshort{dfo}!model-based|)}
...
This phenomenon is studied in~\cite{Gould_Scott_2016}, which presents examples on which comparisons using performance profiles should be made with care.\index{\glsfmtshort{dfo}|)}
...
\begin{enumerate}
    \item \Cref{ch:interpolation} presents interpolation models that are useful to \gls{dfo}.
    ...
    \item \Cref{ch:pdfo} introduces \gls{pdfo}, a package providing MATLAB and Python interfaces for using late Prof.\ M. J. D. Powell's \gls{dfo} solvers.
    \item \Cref{ch:sqp} first presents an overview of the \gls{sqp} method, before providing some perspectives.
    In particular, we provide an interpretation of the \gls{sqp} subproblem that is new to the best of our knowledge.
    We also present a general trust-region \gls{sqp} method, discuss different alternatives for solving its subproblem, and show an interesting connection between two of them.
    \item \Cref{ch:cobyqa-introduction} presents our new model-based \gls{dfo} method \gls{cobyqa}.
    ...
    \item \Cref{ch:cobyqa-subproblems} introduces the methods employed by \gls{cobyqa} to solve its different subproblem.
    \item \Cref{ch:cobyqa-implementation} provides some insight on the Python implementation of \gls{cobyqa}, together with some numerical experiments.
\end{enumerate}
