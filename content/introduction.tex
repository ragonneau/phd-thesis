...
\begin{algorithm}[htp]
    ...
    Initialize $A^{(1)} \gets A$\;
    \For{$k = 1, 2, \dots, n - 1$}{
        Determine the pivot index~$j = \argmax \set[\big]{\abs[\big]{A_{i, k}^{(k)}} : k \le i \le n}$\;
        \eIf{$A_{j, k}^{(k)} \neq 0$}{
            Exchange the~$k$th and the~$j$th rows of~$A^{(k)}$\;
            Evaluate the multiplier~$\tau^{(k)} \in \R^n$ with components
            \begin{algomathdisplay}
                \tau_i^{(k)} =
                \begin{cases}
                    A_{i, k}^{(k)} / A_{k, k}^{(k)} & \text{if~$i > k$, and}\\
                    0                               & \text{otherwise}
                \end{cases}
            \end{algomathdisplay}
            Update~$A^{(k + 1)} \gets (I_n - \tau^{(k)} e_k^{\mathsf{T}})A^{(k)}$\;
        }{
            Set~$A^{(k + 1)} \gets A^{(k)}$\;
        }
    }
...
\begin{equation}
    ...
    \rho_n(A) \eqdef \frac{\max \set{\abs{A_{i, j}^{(k)}} : 1 \le i, j \le n, ~ 1 \le k \le n - 1}}{\max \set{\abs{A_{i, j}} : 1 \le i, j \le n}},
\end{equation}
determinates the numerical stability of~\cref{alg:gepp} because the~$\ell_{\infty}$-norm of the backward error of the computed solution is bounded from above by a term proportional to~$\rho_n(A)$.
We wish to determine how large~$\rho_n(A)$ can be and hence, to solve
\begin{equation}
    \label{eq:gepp-opti}
    \max_{A \in \R^{n \times n}} \rho_n(A).
\end{equation}
Although the growth factor is defined everywhere, it may not be continuous at the points yielding a tie in the selection of the pivot element.
...
In such a case, \gls{dfo} methods can help to determine the optimal solution to problem~\cref{eq:gepp-opti}, and \citeauthor{Higham_Higham_1989} have used \gls{dfo} methods to determine the set of matrices yielding the optimal solution~\cite{Higham_Higham_1989}.
