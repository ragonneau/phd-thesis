...
\begin{subequations}
    \label{eq:kkt-description-one-constraint}
    \begin{align}
    ...
    \begin{tikzpicture}
        \begin{axis}[%
            ...
            axis background/.style={%
                ...
                even odd rule,%
                insert path={let \p1=(axis cs:0,0), \p2=(axis cs:1,1), \n1={veclen(\x2-\x1,\y2-\y1)}, in (\p1) circle[radius=\n1]},%
            },%
        ]
            \draw[dashed] (0,0) circle[radius=2^0.5];
            \draw[-latex] (-1,-1) -- (-3,-3) node[below right] {$\nabla \con{1}(x^{\ast})$};
            \draw[-latex] (-1,-1) -- (-2,-2) node[below right] {$\nabla \obj(x^{\ast})$};
            \addplot[BrickRed,mark=*,only marks] coordinates {(-1,-1)};
        \end{axis}
    \end{tikzpicture}
    \caption{Graphical representation of problem~\cref{eq:kkt-description-one-constraint}}
\end{figure}
...
\begin{figure}[htp]
    ...
    \begin{tikzpicture}
        \begin{axis}[%
            ...
            \draw[dashed] (0,0) circle[radius=2^0.5];
            \draw[dashed] (-4,0) -- (2,0);
            \draw[-latex] (-2^0.5,0) -- (-2*2^0.5,0) node[above] {$\nabla \con{1}(x^{\ast})$};
            \draw[-latex] (-2^0.5,0) -- (-2^0.5,-1) node[below] {$\nabla \con{2}(x^{\ast})$};
            \draw[-latex] (-2^0.5,0) -- (-2^0.5-1,-1) node[below left] {$\nabla \obj(x^{\ast})$};
