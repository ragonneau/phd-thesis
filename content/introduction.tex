...
\todo{Continue the grammar checking from this paragraph}Before solving problem~\cref{eq:problem-introduction}, we must define what a solution is.
...
\begin{definition}[Active set]
    The \emph{active set}~$\act(x) \subseteq \iub \cup \ieq$ for problem~\cref{eq:problem-introduction} at a point~$x \in \R^n$ is defined as
    \begin{equation*}
...
\begin{definition}[Constraint qualifications]
    Given~$x \in \Omega$, denote~$\act(x)$ the active set for problem~\cref{eq:problem-introduction} at~$x$, and assume that the constraint functions~$\con{i}$ are differentiable at~$x$ for all~$i \in \act(x)$.
    We say that
    \begin{itemize}
        ...
        \begin{subequations}
            ...
            \begin{empheq}[left=\empheqlbrace]{alignat=2}
                & \inner{\nabla \con{i}(x), z} < 0  && \quad \text{if~$i \in \act(x) \cap \iub$, and}\\
...
\begin{theorem}[Second-order necessary conditions~\cite{Nocedal_Wright_2006}] % Theorem 12.5
    ...
    Denote the active set for problem~\cref{eq:problem-introduction} at~$x^{\ast}$ by~$\act(x^{\ast})$.
    ...
    \begin{subequations}
        ...
        \begin{empheq}[left=\empheqlbrace]{alignat=2}
            ...
            & \inner{\nabla \con{i}(x^{\ast}), z} = 0,      && \quad \text{if~$i \in \act(x^{\ast}) \cap \iub$ and~$\lambda_i^{\ast} > 0$, and}\\
...
This is because a finite difference at~$x \in \R^n$ require the function to be evaluated on a mesh of~$\bigo(n)$ points around~$x$ oriented along the coordinate directions.
...
A point~$x$ satisfies the test if the reduction~$\varphi_p(x_{\hat{p}}^0) - \varphi_p(x)$ is at least~$1 - \tau$ times the best possible reduction~$\varphi_p(x_p^0) - \varphi_p^{\ast}$ achieved by all solvers under consideration.
We say that a solver solves a problem up to the tolerance~$\tau$ whenever it produces an iterate that achieves the convergence test~\cref{eq:convergence-test-profiles}.
...
Given a threshold~$\alpha \ge 1$ and two solvers~$s_1$ and~$s_2$,~$\rho_{s_1}(\alpha) > \rho_{s_2}(\alpha)$ means that~$s_1$ solves more problems than~$s_2$ under the constraint that~$r_{p, s_1} \le \alpha$ and~$r_{p, s_2} \le \alpha$ for~$p \in \mathcal{P}$.
...
Given a threshold~$\alpha \ge 1$ and two solvers~$s_1$ and~$s_2$,~$d_{s_1}(\alpha) > d_{s_2}(\alpha)$ means that~$s_1$ solves more problems than~$s_2$ under the constraint that~$t_{p, s_1} \le \alpha (n_p + 1)$ and~$t_{p, s_2} \le \alpha (n_p + 1)$ for~$p \in \mathcal{P}$.

\subsection{An illustrative example}

In this section, as an example, we compare three solvers using performance and data profiles.
These solvers, constituting the set~$\mathcal{S}$, are
\begin{itemize}
    ...
    When no derivatives are provided, they use forward finite-difference approximate gradients, with the difference parameter~$h = \sqrt{u}$, where~$u$ is the unit roundoff.
\end{itemize}
The set~$\mathcal{P}$ contains~$154$ smooth unconstrained CUTEst problems of dimension at most~$50$, and the convergence tolerance in~\cref{eq:convergence-test-profiles} is~$\tau = 10^{-3}$.
Since these problems are unconstrained, the merit function~$\varphi_p$ of each problem~$p \in \mathcal{P}$ is set to be the objective function~$\obj_p$.
...
\begin{enumerate}
    \item The first experiment is made without modifying the problems in~$\mathcal{P}$.
    In~\cref{eq:convergence-test-profiles-unconstrained},~$\obj_p^{\ast}$ is set to be the least value of~$f_p$ obtained by all solvers in~$\mathcal{S}$.
    The value of~$t_{p, s}$ is defined to be the number of function evaluations that the solver~$s \in \mathcal{S}$ needs to solve the problem~$p \in \mathcal{P}$ up to the tolerance~$\tau$.
    \item The second experiment is a noisy variation of the previous one.
    Let~$\sigma > 0$ be the noise level.
    For each problem~$p \in \mathcal{P}$, the objective funtion is evaluated by
    \begin{equation}
    ...
    The value of~$t_{p, s}$ is set to the average number of function evaluations that the solver~$s \in \mathcal{S}$ needs to solve the problem~$p \in \mathcal{P}$ up to the tolerance~$\tau$.
...
We can make the following observations on the noise-free experiment according to \cref{fig:performance-profile-example-noiseless}.
\begin{enumerate}
    \item \Gls{bfgs} solves more than~$90\%$ of the problems while the two other solvers solve slightly less problems.
    According to the interpretation in~\cite[\S~2]{More_Wild_2009}, \gls{bfgs} is more reliable than the other two solvers on the test problems in~$\mathcal{P}$, but the margin is narrow.
    \item \Gls{newuoa} uses the least number of function evaluations on about~$50\%$ of the problems, while the percentage for \gls{bfgs} and \gls{cg} are about~$30\%$ and~$25\%$ respectively.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item When~$\sigma = 10^{-10}$, \gls{newuoa} uses the least number of function evaluations on more than~$55\%$ of the problems, while the percentage for \gls{bfgs} and \gls{cg} are about~$20\%$ and~$15\%$ respectively.
    For~$\sigma \in \set{10^{-8}, 10^{-6}}$, \gls{newuoa} is the most efficient on almost all the problems that it can solve, while the other two solvers fail on most of the problems.
\end{enumerate}

Overall, on the noise-free experiment, \gls{bfgs} is slightly more reliable than \gls{newuoa} and \gls{cg}, while \gls{newuoa} is more efficient on about half of the problems.
In contrast, \gls{newuoa} clearly outperforms the two other solvers on noisy problems, although the noise level~$\sigma$ is low while the convergence tolerance~$\tau$ is not demanding.

Note that our results do not contradict the observations of \citeauthor{Shi_Etal_2021}~\cite{Shi_Etal_2021}, where they choose the difference parameter more carefully, according to the noise levels and the smoothness of the problems.
We rather kept the default value provided for the solvers \gls{bfgs} and \gls{cg} in SciPy~1.0~\cite{Virtanen_Etal_2020}.
...
\Gls{newuoa} performs robustly with respect to noise according to this experiment, which agrees with the observations in~\cite{Shi_Etal_2021} on model-based methods.
...
We can make the following observations on the noise-free experiment according to \cref{fig:data-profile-example-noiseless}.
\begin{enumerate}
    \item \Gls{bfgs} solves more than~$90\%$ of the problems while the two other solvers solve slightly less problems.
    This agrees with what is indicated by \cref{fig:performance-profile-example-noiseless}, as it should be the case.
    \item \Gls{bfgs} uses less than~$300 (n_p + 1)$ function evaluations to solve most of the problems~$p \in \mathcal{P}$ that it can solve, while \gls{newuoa} and \gls{cg} uses about~$400 (n_p + 1)$ function evaluations.
\end{enumerate}
...
\begin{enumerate}
    \item \Gls{newuoa} solves always more than~$70\%$ of the problems while the two other solvers solve significantly less problems.
    This agrees with what is indicated by \cref{fig:performance-profile-example-noisy-10,fig:performance-profile-example-noisy-8,fig:performance-profile-example-noisy-6}, as it should be the case.
    \item When~$\sigma = 10^{-10}$, \gls{newuoa} uses less than~$200 (n_p + 1)$ function evaluations to solve most of the problems~$p \in \mathcal{P}$ that it can solve, while \gls{bfgs} and \gls{cg} uses less than~$100 (n_p + 1)$ function evaluations, altough they solve much less problems.
    For~$\sigma \in \set{10^{-8}, 10^{-6}}$, \gls{newuoa} uses less than~$100 (n_p + 1)$ function evaluations to solve most of the problems~$p \in \mathcal{P}$ that it can solve, while the other two solvers fail on most of the problems.
\end{enumerate}

Before concluding this section, we mention a major difference between performance and data profiles.
Comparisons made on data profiles are absolute, and the result of a solver does not influence the result of another, except for the value of~$\varphi_p^{\ast}$ in the convergence test~\cref{eq:convergence-test-profiles}.
On the other hand, comparisons made on data profiles are relative.
The performance of one solver depends on that of all the other solvers, because the value of~$r_{\hat{p}, \hat{s}}$ in~\cref{eq:performance-ratio} depends on~$\set{t_{\hat{p}, s} : s \in \mathcal{S}}$\todo{Rewrite and point out relative and absolute comparisons}.
For example, \cref{fig:data-profile-example-noisy-10} suggests that \gls{cg} has better performance than \gls{bfgs} during the corresponding experiment, although the comparison based on the performance profiles on \cref{fig:performance-profile-example-noisy-10} is not as conclusive.
