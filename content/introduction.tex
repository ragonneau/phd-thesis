...
\nomenclature[Oi]{$\conv$}{Convex hull operator}%
% \nomenclature[Oi]{$\diam$}{Diameter operator}%
...
\nomenclature[Si]{$\iub$}{Set of indices of the inequality constraints}%
\nomenclature[Sj]{$\iub$}{Set of indices of the inequality constraints}%
\nomenclature[Sk]{$\Omega$}{Feasible set, included in~$\R^n$}%
\nomenclature[Sl]{$\ieq$}{Set of indices of the equality constraints}%
\nomenclature[Sm]{$\iub$}{Set of indices of the inequality constraints}%
...
\begin{algorithm}[ht]
    ...
    \For{$k = 0, 1, \dots$}{
        ...
        \begin{algoempheq}[left={\Delta_{k + 1} \gets \empheqlbrace}]{alignat*=2}
            ...
            & \Delta_k,             && \quad \text{if~$\eta_1 < \rho_k \le \eta_2$,}\\
...
\end{equation*}
where~$\lambda = [\lambda_i]_{i \in \iub \cup \ieq}^{\top}$ with~$\lambda_i \in \R$ for all~$i \in \iub \cup \ieq$.
...
\begin{theorem}[First-order necessary conditions~\cite{Nocedal_Wright_2006}] % Theorem 12.1
    ...
    Then there exists a Lagrange multiplier~$\lambda^{\ast} = [\lambda_i^{\ast}]_{i \in \iub \cup \ieq}^{\top}$ with~$\lambda_i^{\ast} \in \R$ for all~$i \in \iub \cup \ieq$ such that
    \begin{subequations}
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
whose solution is~$x^{\ast} = [-\sqrt{2}, 0]^{\top}$.
...
\begin{subequations}
    ...
    \end{empheq}
\end{subequations}
The Farkas' lemma~\cite{Farkas_1902} ensures, therefore, that there exists a nonnegative Lagrange multiplier~$\lambda^{\ast} = [\lambda_1^{\ast}, \lambda_2^{\ast}]^{\top}$ such that
\begin{equation*}
...
\begin{theorem}[Second-order necessary conditions~\cite{Nocedal_Wright_2006}] % Theorem 12.5
    ...
    Let~$\lambda^{\ast} = [\lambda_i^{\ast}]_{i \in \iub \cup \ieq}^{\top}$ be a Lagrange multiplier with~$\lambda_i^{\ast} \in \R$ for all~$i \in \iub \cup \ieq$ satisfying the \gls{kkt} conditions~\cref{eq:kkt-introduction}, and let~$z \in \R^n$ be any vector such that
    \begin{subequations}
...
The \emph{performance profile} of~$s$ is defined for~$\alpha \ge 1$ as
\begin{equation*}
    \rho_s(\alpha) \eqdef \frac{1}{\card(\xpb)} \card (\set{p \in \xpb : r_{p, s} \le \alpha}),
\end{equation*}
...
The \emph{data profile} of a solver~$s \in \xsv$ is defined for~$\alpha \ge 0$ as
\begin{equation}
    \label{eq:data-profile}
    d_s(\alpha) \eqdef \frac{1}{\card(\xpb)} \card \bigg(\set[\bigg]{p \in \xpb : \frac{t_{p, s}}{n_p + 1} \le \alpha}\bigg),
\end{equation}
