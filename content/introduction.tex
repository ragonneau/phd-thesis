...
\begin{subequations}
    ...
    \begin{align}
        \min        & \quad \frac{1}{2} \norm{\omega}_2^2 + C \norm{\xi}_1\\
        \text{s.t.} & \quad y_i \big( \beta + \omega^{\T} \varphi_{\gamma}(\iter_i) \big) \ge 1 - \xi_i, ~ i \in \set{1, 2, \dots, m},\\
                    & \quad \xi \ge 0,\\
...
\begin{equation}
    ...
    \delta(\iter) \eqdef \sgn \big( \beta^{\ast} + (\omega^{\ast})^{\T} \varphi_{\gamma}(\iter) \big),
\end{equation}
