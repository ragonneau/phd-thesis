...
\begin{subequations}
    ...
    \begin{align}
        \min        & \quad \frac{1}{2} \norm{\omega}_2^2 + C \norm{\xi}_1\\
        \text{s.t.} & \quad y_i (\beta + \omega^{\T} \varphi_{\gamma}(x_i)) \ge 1 - \xi_i, ~ i \in \set{1, 2, \dots, m},\\
                    & \quad \xi \ge 0,\\
...
\begin{equation}
    ...
    \delta(x) \eqdef \sgn(\beta^{\ast} + (\omega^{\ast})^{\T} \varphi_{\gamma}(x)),
\end{equation}
    ...
    \begin{itemize}
        ...
        \begin{subequations}
            ...
            \begin{empheq}[left=\empheqlbrace]{alignat=2}
                & \nabla \con{i}(x)^{\T} z < 0, && \quad \text{if~$i \in \act(x) \cap \iub$,}\\
                & \nabla \con{i}(x)^{\T} z = 0, && \quad \text{if~$i \in \ieq$.}
            \end{empheq}
        \end{subequations}
    \end{itemize}
\end{definition}
...
\begin{subequations}
    ...
    \begin{empheq}[left=\empheqlbrace]{alignat=1}
        & \nabla \obj(x^{\ast})^{\T} d < 0,\\
        & \nabla \con{1}(x^{\ast})^{\T} d \le 0,\\
        & \nabla \con{2}(x^{\ast})^{\T} d \le 0.
    \end{empheq}
\end{subequations}
...
\begin{theorem}[Second-order necessary conditions~{\cite[thm.~12.5]{Nocedal_Wright_2006}}]
    ...
    \begin{subequations}
        ...
        \begin{empheq}[left=\empheqlbrace]{alignat=2}
            & \nabla \con{i}(x^{\ast})^{\T} z = 0,      && \quad \text{if~$i \in \ieq$,}\\
            & \nabla \con{i}(x^{\ast})^{\T} z = 0,      && \quad \text{if~$i \in \act(x^{\ast}) \cap \iub$ and~$\lambda_i^{\ast} > 0$,}\\
            & \nabla \con{i}(x^{\ast})^{\T} z \ge 0,    && \quad \text{if~$i \in \act(x^{\ast}) \cap \iub$ and~$\lambda_i^{\ast} = 0$.}
        \end{empheq}
    \end{subequations}
