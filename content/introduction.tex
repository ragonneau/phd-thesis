...
\todo[noline]{If~$x$ is not optimal, then the system is impossible and thus, the Farkas lemma concludes the proof}

We do not provide a proof of~\cref{thm:first-order-necessary-conditions}, but we illustrate graphically the main idea on the simple example
\begin{subequations}
...
\citeauthor{Rosenbrock_1960}, for example, designed a direct-search method, which chooses the search directions among linear combinations of previous steps that led to actual reductions of a merit function\todo{The concept of merit functions is not yet introduced}~\cite{Rosenbrock_1960}.
...
An example of performance profiles is given in~\cref{fig:performance-profile-example}\todo{What is the experiment?}.

\begin{figure}[ht]
    \centering
    \drawprofiles{{"NEWUOA","CG"}}{plain-1-50-perf-cg-newuoa-U.csv}{5}
