...
\todo[noline,color=OliveGreen!50]{Rewrite it all without stories and make it short}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        To the best of our knowledge, this is the first trust-region \gls{sqp} algorithm that handles \gls{dfo} problems with equality and inequality constraints.
        ...
        \item We present in \cref{ch:cobyqa-implementation} the Python implementation of \gls{cobyqa}, which is open-source and publicly available at \url{https://www.cobyqa.com/}\todo{Check it}.
        % We note that this implementation is highly nontrivial due to the nature of \gls{dfo}.
        The numerical experiments demonstrate that \gls{cobyqa} is competitive with \gls{newuoa} and \gls{bobyqa} on unconstrained and bound-constrained problems, respectively, while being able to tackle more general problems.
        On linearly-constrained problems, the current version of \gls{cobyqa} does not perform as well as \gls{lincoa}.
        However, \gls{cobyqa} has the strength that it always respects the bound constraints, while \gls{lincoa} does not.
        Such a strength is crucial in many applications, as is requested by engineers from IRT Saint Exup{\'{e}}ry.
        Most importantly, \gls{cobyqa} provides much better performance than \gls{cobyla} on all types of problems, even though \gls{cobyla} does not always respect the bound constraints (if any).
