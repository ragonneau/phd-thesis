...
To choose the parameters~$\eta_1$,~$\eta_2$,~$\theta_1$, and~$\theta_2$, we minimize some measure of the method's expense (e.g., the \glsxtrshort{cpu} time to solve a given set of problems), which will be denoted by~$C$.
...
Such a problem can be solved using \gls{dfo} methods, such as the \gls{mads} method~\cite{Audet_Orban_2006,Audet_Digabel_Tribes_2019}.
