...
\begin{enumerate}
    \item \Gls{newuoa} solves always more than \SI{70}{\percent} of the problems while the two other solvers solve significantly less problems.
...
\begin{enumerate}
    \item \Gls{bfgs} solves more than \SI{90}{\percent} of the problems while the two other solvers solve slightly less problems.
...
\begin{enumerate}
    \item \Gls{newuoa} solves always more than \SI{70}{\percent} of the problems while the two other solvers solve significantly fewer problems.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        They differ from the extensions given in~\cite[\S~15.4.4]{Conn_Gould_Toint_2000}.
    ...
    \begin{enumerate}
        ...
        To the best of our knowledge, this is the first trust-region \gls{sqp} algorithm that can solve \gls{dfo} problems with nonlinear equality and inequality constraints.
        \item We detail in \cref{ch:cobyqa-subproblems} the algorithms for solving the subproblems of \gls{cobyqa} in our implementation.
        ...
        \item We present in \cref{ch:cobyqa-implementation} the Python implementation of \gls{cobyqa}, which is open-source and publicly available at \url{https://www.cobyqa.com/}.
        \Cref{sec:cobyqa-experiments} details numerical experiments that compare \gls{cobyqa} with Powell's solvers.
        % We note that this implementation is highly nontrivial due to the nature of \gls{dfo}.
        \Gls{cobyqa} is competitive with \gls{newuoa} and \gls{bobyqa} on unconstrained and bound-constrained problems, respectively, while being able to tackle more general problems.
        ...
        % However, the current version of \gls{cobyqa} does not perform as well as \gls{lincoa}.
        ...
        % though \gls{cobyla} does not always respect the bound constraints.
        As a general-purpose solver, \gls{cobyqa} is a good successor to \gls{cobyla}.
        % Indeed, implementing \gls{cobyqa} requires implementing many components (trust-region subproblem solvers, updating of quadratic models, etc) that must be correct independently, but also work together when integrating them.
    \end{enumerate}
\end{enumerate}
