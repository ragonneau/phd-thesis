...
\nomenclature[Oh]{$\circ$}{Hadamard product}%
...
\nomenclature[Sk]{$\fset$}{Feasible set, included in~$\R^n$}%
...
\begin{equation*}
    \lag(\iter, \lambda) \eqdef \obj(\iter) + \sum_{\mathclap{i \in \iub \cup \ieq}} \lambda_i \con{i}(\iter), \quad \text{for~$\iter \in \R^n$ and~$\lambda_i \in \R$ for~$i \in \iub \cup \ieq$},
\end{equation*}
...
\begin{theorem}[First-order necessary conditions~{\cite[thm.~12.1]{Nocedal_Wright_2006}}]
    ...
    \begin{subequations}
        ...
        \begin{empheq}[left=\empheqlbrace]{alignat=2}
            & \nabla_x \lag(\iter[\ast], \lambda^{\ast}) = 0,  && \label{eq:kkt-introduction-stationarity}\\
            ...
            & \lambda_i^{\ast} \con{i}(\iter[\ast]) = 0,       && \quad \text{if~$i \in \iub$,} \label{eq:kkt-introduction-complementary-slackness}\\
...
\begin{equation*}
    \nabla \obj(\iter[\ast]) + \lambda_1^{\ast} \nabla \con{1}(\iter[\ast]) + \lambda_2^{\ast} \nabla \con{2}(\iter[\ast]) = 0.
\end{equation*}
...
\begin{theorem}[Second-order necessary conditions~{\cite[thm.~12.5]{Nocedal_Wright_2006}}]
    ...
    \begin{subequations}
        ...
        \begin{empheq}[left=\empheqlbrace]{alignat=2}
            ...
            & \nabla \con{i}(\iter[\ast])^{\T} z = 0,      && \quad \text{if~$i \in \act(\iter[\ast]) \cap \iub$ and~$\lambda_i^{\ast} > 0$,}\\
            & \nabla \con{i}(\iter[\ast])^{\T} z \ge 0,    && \quad \text{if~$i \in \act(\iter[\ast]) \cap \iub$ and~$\lambda_i^{\ast} = 0$.}
        \end{empheq}
    \end{subequations}
    Then~$z^{\T} \nabla_{x, x}^2 \lag(\iter[\ast], \lambda^{\ast}) z \ge 0$.
\end{theorem}
...
\begin{theorem}[Second-order sufficient conditions~{\cite[thm.~12.6]{Nocedal_Wright_2006}}]
    ...
    If for all vector~$z \in \R^n \setminus \set{0}$ satisfying the conditions~\cref{eq:second-order-introduction} we have~$z^{\T} \nabla_{x, x}^2 \lag(\iter[\ast], \lambda^{\ast}) z > 0$, then~$\iter[\ast]$ is a strict local solution to problem~\cref{eq:problem-introduction}.
\end{theorem}
