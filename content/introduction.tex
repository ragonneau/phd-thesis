...
We emphasize that we are \emph{not} investigating nonsmooth optimization~\cite{Clark_1983,Bagirov_Karmitsa_Makela_2014}\todo[color=OliveGreen!50]{Put two classical references}, which studies problems that involve nonsmooth functions.
...
In practice, each function evaluation may require several minutes or even several days to complete~\cite[\S 1.4]{Audet_Hare_2017}\todo[color=OliveGreen!50]{Refer to the section in a book that talks about this}.
...
We will later in this thesis introduce a new \gls{dfo} method, which will take this into consideration\todo[color=OliveGreen!50]{Find a better word} (see~\cref{ch:cobyqa-intro}).
...
\Gls{dfo} methods are also widely used in engineering, especially for solving design problems, including heavy simulations.
Examples of such problems include helicopter rotor blade design~\cite{Booker_Etal_1998a,Booker_Etal_1998b,Serafini_1998}, aeroacoustic shape design~\cite{Marsden_2004,Marsden_Etal_2004}, hydrodynamic design~\cite{Duvigneau_Visonneau_2004}, analog circuit design~\cite{Latorre_Etal_2019}, and rapid-cycling synchrotron design~\cite{Eldred_Etal_2021}.
More generally, industrial and engineering problems that involve sophisticated models, complex simulations, or experiments, give rise to \gls{dfo} problems.
This is true in many areas, such as in nuclear energy engineering~\cite{Kortelainen_Etal_2010,Kortelainen_Etal_2012,Kortelainen_Etal_2014}, reservoir engineering and engine calibration~\cite{Langouet_2011}, or groundwater supply and bioremediation engineering~\cite{Fowler_Etal_2008,Mugunthan_Shoemaker_Regis_2005,Yoon_Shoemaker_1999}, for instance.
Another interesting application is \gls{mdo}.
It is a field in industry and engineering that uses optimization methods for solving design problems whose components are defined by several disciplines.
...
Because of the attraction of \gls{mdo} for practitioners, several dedicated packages are available on the market for solving \gls{mdo} problems numerically.
We will present in~\cref{ch:pdfo} of this thesis a piece of software we implemented for solving \gls{dfo} problems, which has been included in GEMSEO~\cite{Gallard_Etal_2018}, an engine for \gls{mdo} initiated by a team from IRT Saint Exup{\'{e}}ry in France.
...
\Cref{def:local-solution} provides another understanding of solutions to problem~\cref{eq:nlcp-intro}.
...
However, we note that the numerical methods we will consider in this thesis do \emph{not} attempt to find any local solution to~\cref{eq:nlcp-intro}, as such problem is also NP-hard in general~\cite{Murty_Kabadi_1987}.
...
We do not provide a rigorous of~\cref{thm:fonc}, but we illustrate the main steps on a simple example.
...
\todo[noline,color=OliveGreen!50]{Find a solution to make the proof logic}
...
\end{equation*}
A formal proof of~\cref{thm:fonc} (see, e.g.,~\cite[\S 12.4]{Nocedal_Wright_2006}) follows the scheme presented above, by bounding the error terms arising in the first-order approximation~\cref{eq:kkt-description-system}.
...
Essentially two main strategies have been developed in the litterature for solving \gls{dfo} problems.
One strategy consists in sampling iteratively the objective function at a finite number of points around the current iterate, and choosing among these points the next iterate.
...
These methods are referred to as the \emph{model-based} methods.
The approximated problems are usually embedded in a globalization strategy, such as a \emph{line-search} method or a \emph{trust-region} methods~\cite{Conn_Gould_Toint_2000,Yuan_2015}, to globalize their convergence properties.
...
Some methods cannot be classified as direct-search or model-based, but are rather hybridized, such as the implicit filtering~\cite{Kelley_2011}, the SID-PSM method~\cite{Custodio_Rocha_Vicente_2009}.
...
At the end of the millennium, with the prominent works of \citeauthor{Wright_1995} and others, \gls{dfo} became the wide research area that exists today, encouraged by recent engineering needs.
