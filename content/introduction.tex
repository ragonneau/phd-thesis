...
This problem may be solved numerically using \gls{dfo} methods.

In a similar fashion\todo{Rewrite and give more details}, reinforcement learning in machine learning necessitates solving optimization problems for which derivatives cannot be assessed~\cite{Qian_Yu_2021}.
...
% Moreover, the smoothness assumptions on the objective and constraint functions can be relaxed, using generalized derivatives.

\subsubsection{Graphical insight of the first-order optimality conditions}

Consider the two-dimensional problem
\begin{subequations}
    \label{eq:kkt-description-two-constraint}
    \begin{align}
        \min        & \quad \obj(x) = x_1 + x_2\\
        \text{s.t.} & \quad \con{1}(x) = x_1^2 + x_2^2 - 2 \le 0,\\
                    & \quad \con{2}(x) = -x_2 \le 0,\\
                    & \quad x \in \R^2. \nonumber
    \end{align}
\end{subequations}
...
\begin{itemize}
    \item If an inequality constraint is not active at~$x^{\ast}$, then removing this constraint from the problem does not modify its solution, and the corresponding Lagrange multiplier coefficient must be zero.
\end{itemize}
