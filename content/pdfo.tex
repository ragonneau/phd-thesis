...
In this chapter, we present \gls{pdfo}, a MATLAB and Python software we developed to interface Powell's solvers, which has been downloaded more than 30,000 times as of today.
After briefly introducing the techniques employed by the solvers, we present the core features of \gls{pdfo} and its implementation.
At the end of the chapter, we present some numerical experiments on Powell's solvers using \gls{pdfo}, showing the advantage of having all five solvers in one package.
One of these experiments will reveal a peculiar behavior of the solvers, namely that \gls{cobyla} defeats all other methods on noisy unconstrained problems.
This behavior is unexpected, because \gls{cobyla} is the oldest of these methods and uses the simplest models.
...
We observe, however, a peculiar behavior of \gls{cobyla} in this experiment, as it defeats all other solvers on unconstrained problems even though it is not defined for such kind of problem and uses the simplest models.
