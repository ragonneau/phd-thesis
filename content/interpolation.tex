...
% In other words, would a slight perturbation to the interpolation points generate a nonpoised set?
...
% \todo[noline]{Comment the whole next section and explain it simply in one sentence}

% \subsection{Conditioning of the interpolation system}

% As mentioned above, the~$\Lambda$-poisedness measures how good an interpolation set is.
% In this section, we relate this notion with the condition number of a particular matrix, explaining more intrinsically why a~$\Lambda$-poised set can be considered as a good set if~$\Lambda$ is small.

% The system~\cref{eq:interpolation-conditions-quadratic} can be formulated as
% \begin{equation}
%     \label{eq:quadratic-interpolation-conditions}
%     \alpha + \inner{g, y^i} + \frac{1}{2} \inner{y^i, Hy^i} = \obj(y^i), \quad \text{for~$i \in \set{1, 2, \dots, m}$},
% \end{equation}
% where~$\alpha \in \R$,~$g \in \R^n$, and~$H \in \R^{n \times n}$ are the coefficients of~$\objm$, with~$H$ being symmetric.
% Let~$M \in \R^{m \times m}$ be the matrix
% \begin{equation*}
%     M \eqdef
%     \begin{bmatrix}
%         1       & y_1^1     & y_2^1     & \cdots    & y_n^1     & (y_1^1)^2 / 2 & y_1^1 y_2^1   & \cdots    & (y_n^1)^2 / 2\\
%         1       & y_1^2     & y_2^2     & \cdots    & y_n^2     & (y_1^2)^2 / 2 & y_1^2 y_2^2   & \cdots    & (y_n^2)^2 / 2\\
%         \vdots  & \vdots    & \vdots    & \ddots    & \vdots    & \vdots        & \vdots        & \ddots    & \vdots\\
%         1       & y_1^m     & y_2^m     & \cdots    & y_n^m     & (y_1^m)^2 / 2 & y_1^m y_2^m   & \cdots    & (y_n^m)^2 / 2
%     \end{bmatrix},
% \end{equation*}
% where~$y_i$ denotes the~$i$th component of~$y \in \R^n$ for all~$i \in \set{1, 2, \dots, n}$.
% The system~\cref{eq:quadratic-interpolation-conditions} is equivalent to
% \begin{equation*}
%     M
%     \begin{bmatrix}
%         \alpha\\
...
%     = \obj(\xpt),
% \end{equation*}
% where~$\obj(\xpt) \eqdef (f(y^1), f(y^2), \dots, f(y^m))$, and where~$v \in \R^{n(n + 1) / 2}$ is the vector that contains the upper-triangular part of~$H$, columnwise.
% Clearly, the matrix~$M$ is nonsingular if and only if the interpolation set~$\xpt$ is poised.

% In the following,~$\cond(M)$ denotes the spectral condition number of~$M$.
% The following theorem highlights the relationships between the~$\Lambda$-poisedness of the interpolation set~$\xpt$ and~$\cond(M)$.

% \begin{theorem}
%     \label{thm:lambda-poisedness-conditioning}
%     Assume that the interpolation set~$\xpt$ satisfies~$y^1 = 0$, and
%     \begin{equation*}
%         \max_{1 \le i \le m} \norm{y^i} = 1.
%     \end{equation*}

%     \begin{equation*}
%         \theta m^{-2} \cond(M) \le \Lambda \le \sqrt{m} \cond(M).
%     \end{equation*}
%     If the above-defined matrix~$M$ is nonsingular and~$\cond(M) \le \kappa$, then the interpolation set~$\xpt$ is~$\sqrt{m} \kappa$-poised in~$\ball[2](1)$.
%     Moreover, if the interpolation set~$\xpt$ is~$\Lambda$-poised in~$\ball[2](1)$, then the matrix~$M$ is nonsingular and
%     \begin{equation*}
%         \cond(M) \le \theta m^2 \Lambda,
%     \end{equation*}
...
% According to \cref{thm:lambda-poisedness-conditioning}, the lower is~$\Lambda$, the better is the conditioning of the interpolation system and hence, the better is the interpolation set from a numerical standpoint.
% If~$\xpt$ is~$\Lambda$-poised in a compact set~$\mathcal{C} \subseteq \R^n$ and~$\Lambda$ is reasonable low, then we thus can expect the quadratic model~$\objm$ to represent reasonably well~$\obj$ in~$\mathcal{C}$.
...
% For more properties of the~$\Lambda$-poisedness of an interpolation set, we refer to~\cite[\S~3.3]{Conn_Scheinberg_Vicente_2009b}.
...
% We do not change the notations with the case where~$m = \dim \qpoly$, because underdetermined interpolation models generalize the former.
...
Examples of \gls{dfo} methods that use least Frobenius norm quadratic models include \gls{mnh}~\cite{Wild_2008}\todo{Add more examples (DFO)}.
...
% \begin{proposition}
%     If the set~$\xpt$ is poised in the minimum Frobenius norm sense, then the solution to the problem~\cref{eq:least-frobenius-norm-models} is unique.
% \end{proposition}

% \begin{proof}
%     Assume that~\cref{eq:least-frobenius-norm-models} admits two distinct solutions~$Q_1$ and~$Q_2$.
%     The strict convexity of the Frobenius norm shows directly that~$\nabla^2 Q_1 = \nabla^2 Q_2$.
% \end{proof}
...
\begin{equation*}
    Y_H \eqdef \frac{1}{2}
    \begin{bmatrix}
        \norm{y^1}^4        & \inner{y^1, y^2}^2    & \cdots    & \inner{y^1, y^m}^2\\
        \inner{y^2, y^1}^2  & \norm{y^2}^4          & \cdots    & \inner{y^2, y^m}^2\\
...
Throughout this section, we fix a poised interpolation set~$\xpt \subseteq \R^n$ of~$m$ points with~$n + 2 \le m \le \dim \qpoly$.
...
We now present the notion of~$\Lambda$-poisedness in the minimum Frobenius norm sens, which is a straightforward adaptation of \cref{def:lambda-poisedness}.
...
Moreover, one can show that the set~$\xpt$ is~$\Lambda$-poised if and only if the condition number of the coefficient matrix in~\cref{eq:least-frobenius-kkt-system} is bounded by some terms proportional to~$\Lambda$~\cite[thm.~5.8]{Conn_Scheinberg_Vicente_2009b}.å
...
\begin{equation*}
    \objm[k] = \alpha^k + \inner{g^k, x - x^k} + \frac{1}{2} \inner{x - x^k, H^k (x - x^k)}.
\end{equation*}
where~$\alpha^k \in \R$,~$g^k \in \R^n$, and~$H^k \in \R^{n \times n}$ are the coefficients of the model,~$H$ being symmetric
By adapting the calculations made in~\cref{eq:least-frobenius-kkt-system}, we can show that the coefficients of~$\objm[k]$ can be evaluated by solving
\begin{equation}
    \label{eq:least-frobenius-kkt-system-offset}
    \begin{bmatrix}
        Y_H         & Y_L\\
        Y_L^{\T}    & 0
    \end{bmatrix}
...
\begin{equation*}
    ...
    \end{bmatrix},
\end{equation*}
the former being chosen if we want a least Frobenius norm model and the latter if we want to update the model by a \gls{dfo} symmetric Broyden update, and where
\begin{equation*}
    Y_H \eqdef \frac{1}{2}
    \begin{bmatrix}
        \norm{y^{k, 1} - x^k}^4                     & \inner{y^{k, 1} - x^k, y^{k, 2} - x^k}^2  & \cdots    & \inner{y^{k, 1} - x^k, y^{k, m} - x^k}^2\\
        \inner{y^{k, 2} - x^k, y^{k, 1} - x^k}^2    & \norm{y^{k, 2} - x^k}^4                   & \cdots    & \inner{y^{k, 2} - x^k, y^{k, m} - x^k}^2\\
        \vdots                                      & \vdots                                    & \ddots    & \vdots\\
        \inner{y^{k, m} - x^k, y^{k, 1} - x^k}^2    & \inner{y^{k, m} - x^k, y^{k, 2} - x^k}^2  & \cdots    & \norm{y^{k, m} - x^k}^4\\
    \end{bmatrix}
\end{equation*}
and
\begin{equation*}
    Y_L \eqdef
    \begin{bmatrix}
        1       & y_1^{k, 1} - x^k  & y_2^{k, 1} - x^k  & \cdots    & y_n^{k, 1} - x^k\\
        1       & y_1^{k, 2} - x^k  & y_2^{k, 2} - x^k  & \cdots    & y_n^{k, 2} - x^k\\
        \vdots  & \vdots            & \vdots            & \ddots    & \vdots\\
        1       & y_1^{k, m} - x^k  & y_2^{k, m} - x^k  & \cdots    & y_n^{k, m} - x^k\\
    \end{bmatrix}.
\end{equation*}
Further, if we want a least Frobenius norm model, we set
...
In practice, we often maintain the inverse of the coefficient matrix in~\cref{eq:least-frobenius-kkt-system-offset} to evaluates the coefficients of the model~$\objm[k]$.
...
The intercepts and the gradient vectors of the models can be stored directly, as they are directly evaluated or updated by the linear system~\cref{eq:least-frobenius-kkt-system-offset}.
However, solving this system does not provide directly the Hessian matrix, but a decomposition of it.
Therefore, as suggested by~\cite[\S~3]{Powell_2004b}, the Hessian matrix of the model is stored as
\begin{equation}
...
As we mentioned earlier, the inverse of the coefficient matrix in~\cref{eq:least-frobenius-kkt-system-offset} can be maintained by the \gls{dfo} method we consider.
...
\begin{enumerate}
    ...
    \item As mentioned by \citeauthor{Powell_2004c}~\cite{Powell_2004c}, the leading~$m \times m$ submatrix of the inverse of the coefficient matrix in~\cref{eq:least-frobenius-kkt-system-offset} has rank~$m - n - 1$ and is positive semi-definite.
    ...
    Therefore, this submatrix is not stored directly, but his stored using the rank factorization~$Z D Z^{\top}$, with~$Z \in \R^{m \times (m - n - 1)}$ and~$D \in \R^{(m - n - 1) \times (m - n - 1)}$, where~$D$ is a diagonal matrix with elements~$\pm 1$ on the diagonal.
    From the theoretical standpoint, the matrix~$D$ should always be identity.
    However, in practice, allowing the leading~$m \times m$ submatrix of the inverse of the coefficient matrix in~\cref{eq:least-frobenius-kkt-system-offset} to have negative eigenvalues prevent damages from computer rounding errors.
...
The methods we consider in this thesis that employ the inverse of the coefficient matrix in~\cref{eq:least-frobenius-kkt-system-offset} build~$\xpt[k + 1]$ from~$\xpt[k]$ by modifying only one point.
Therefore, only one row and one column of the coefficient matrix in~\cref{eq:least-frobenius-kkt-system-offset} is modified from an iteration to another.
This modification leads to an at-most rank-$2$ modification of the inverse of this matrix.
Hence, we build do not compute explicitely the inverse matrix at each iteration, but we update it using the Sherman-Morrison-Woodbury formula.
...
\end{empheq}
This also motivates the decomposition~\cref{eq:hessian-explicit-implicit}.
...
% \todo{Shorten the proof but keep the original version as a remark}.
