...
\todo[noline,color=OliveGreen!50]{Write the conclusion for \cref{ch:interpolation}}

In this chapter, we presented concepts of polynomial interpolation that we will apply later in this thesis to model-based \gls{dfo} methods.

We first introduced fully-determined linear and quadratic interpolation.
We saw that an interpolation set is poised if the determinant of the interpolation system is nonzero.
We also presented the quadratic Lagrange polynomials on poised interpolation sets.
These polynomials are particularly useful in a \gls{dfo} context because model-based methods often rely on them to improve the geometry of their interpolation sets.
This will be the case with \gls{cobyqa}, the new \gls{dfo} method we will introduce in \cref{ch:cobyqa-introduction}.
We then introduced the concept of~$\Lambda$-poisedness.
We remarked that an interpolation set is~$\Lambda$-poised if and only if the condition number of the interpolation system is bounded by some term proportional to~$\Lambda$.
This is useful because it measures to some extent how close the interpolation set is to degeneracy.
In the meantime, a low condition number of the interpolation system prevents numerical difficulties arising in computations due to computer rounding errors.
...
We extended the concepts of poisedness, quadratic Lagrange polynomials, and~$\Lambda$-poisedness to underdetermined interpolation.
We also discussed how to implement such underdetermined quadratic models in \gls{dfo} methods.
In particular, we saw that when a method updates one point at a time in the interpolation set, it was possible and advantageous to maintain the inverse of the coefficient matrix of the interpolation problem, because modifying a point in the interpolation sets leads to an at-most rank-$2$ modification of this matrix.
We will use this mechanism in \gls{cobyqa}.

Finally, we studied in this chapter the~$\Lambda$-poisedness of a particular interpolation set.
...
We also believe that the results we presented can be extended to the general case, as we assumed here that the number of interpolation points was at most~$2n + 1$.
