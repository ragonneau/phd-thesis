...
\begin{align*}
    \objm(x) = \objm(y^1) + \nabla \objm(y^1)^{\T} (x - y^1) + \frac{1}{2} (x - y^1)^{\T} H (x - y^1),
\end{align*}
...
Further, it builds the model~$\objm[k] \in \qpoly$ of the objective function~$\obj$ by an underdetermined interpolation scheme on the interpolation set~$\xpt[k] = \set{y^{k, 1}, y^{k, 2}, \dots, y^{k, m}} \subseteq \R^n$, with~$x^k \in \xpt[k]$.
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
for some~$v \in \R^m$, detailed below.

\subsubsection{Minimum Frobenius norm model}

As mentioned earlier, to obtain a minimum Frobenius norm model, we set
\begin{equation*}
    v \eqdef
    \begin{bmatrix}
        \obj(y^{k, 1})\\
    ...
    \end{bmatrix}.
\end{equation*}
Further, we clearly set~$\alpha^k = \alpha$,~$g^k = g$, and
\begin{equation*}
    H^k = \sum_{i = 1}^m \lambda_i (y^{k, i} - x^k) (y^{k, i} - x^k)^{\T}.
\end{equation*}

\subsubsection{Model based on the derivative-free symmetric Broyden update}

Thanks to~\cref{eq:symmetric-broyden-models-reformulation}, to build a model based on the derivative-free symmetric Broyden update, we set
\begin{equation*}
    v \eqdef
    \begin{bmatrix}
    ...
    \end{bmatrix}.
\end{equation*}
In such a case, the model's coefficients are
...
We present in what follows techniques that are used to reduce the computational cost of computing these models and to improve the stability of the methods.
