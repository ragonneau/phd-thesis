...
\end{equation}
In~\cref{eq:least-frobenius-kkt-system-matrices},~$\circ$ denotes the Hadamard product and hence, the~$(i, j)$ entry of~$Y_H$ is given by~$\inner{y^i - \bar{x}, y^j - \bar{x}}^2 / 2$.
...
This is because~$\alpha + \inner{g, x - \bar{x}}$ is a nonzero linear polynomial that interpolates the zero function on~$\xpt$ if~$[\alpha, g^{\T}]^{\T} \in \R^{n + 1}$ is a nonzero vector in~$\ker([e, Y_L^{\T}])$.
...
The minimum Frobenius norm update can be regarded as a derivative-free variation of the symmetric Broyden update in the quasi-Newton method.
The symmetric Broyden method~\cite{Powell_1970b} approximates the function~$\obj$ at a point~$y^1 \in \R^n$ by the quadratic function
\begin{align*}
    \objm(x) = \objm(y^1) + \inner{\nabla \objm(y^1), x - y^1} + \frac{1}{2} \inner{x - y^1, H (x - y^1)},
\end{align*}
where, as shown in~\cite[thm.~4.2]{Dennis_Schnabel_1979},~$H \in \R^{n \times n}$ is the unique solution to
\begin{align}
...
In an optimization context,~$y^1$ and~$y^2$ are respectively the current and the previous iterate generated by the optimization method.
...
We assume in this section that we are given a model-based \gls{dfo} method, and we denote by~$x^k \in \R^n$ the~$k$th iterate, with~$k \ge 0$.
Further, we assume that it builds the model~$\objm[k] \in \qpoly$ of the objective function~$\obj$ by an underdetermined interpolation scheme on the interpolation set~$\xpt[k] = \set{y^{k, 1}, y^{k, 2}, \dots, y^{k, m}} \subseteq \R^n$, with~$x^k \in \xpt[k]$.
The quadratic model~$\objm[k]$ is
\begin{equation*}
    \objm[k](x) = \alpha^k + \inner{g^k, x - x^k} + \frac{1}{2} \inner{x - x^k, H^k (x - x^k)}, \quad \text{for~$x \in \R^n$}.
\end{equation*}
where~$\alpha^k \in \R$,~$g^k \in \R^n$, and~$H^k \in \R^{n \times n}$ are the coefficients of the model,~$H^k$ being symmetric.
As we mentioned in~\cref{eq:least-frobenius-kkt-system}, the coefficients of~$\objm[k]$ can be evaluated by solving
\begin{equation}
...
\begin{equation*}
    ...
    \end{bmatrix},
\end{equation*}
the former being chosen if we want a least Frobenius norm model and the latter if we want to update the model by a derivative-free symmetric Broyden update, and where the matrices~$Y_H \in \R^{m \times m}$ and~$Y_L \in \R^{n \times m}$ are defined by~\cref{eq:least-frobenius-kkt-system-matrices}.
...
\end{equation*}
and if we want a model obtained by the derivative-free symmetric Broyden update, we set
...
\begin{enumerate}
    ...
    Therefore, this submatrix is not stored directly, but using the rank factorization~$Z D Z^{\T}$, with~$Z \in \R^{m \times (m - n - 1)}$ and~$D \in \R^{(m - n - 1) \times (m - n - 1)}$, where~$D$ is a diagonal matrix with elements~$\pm 1$ on the diagonal.
...
\end{empheq}
and if we are updating the model using the derivative-free symmetric Broyden update, we can set
\begin{empheq}[left=\empheqlbrace]{alignat*=2}
