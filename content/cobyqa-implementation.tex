...
\begin{empheq}[left={\varphi(\iter) = \empheqlbrace}]{alignat*=2}
    & \obj(\iter),                                  && \quad \text{if~$v_{\infty}(\iter) \le 10^{-14}$,}\\
    & \infty,                                       && \quad \text{if~$v_{\infty}(\iter) \ge 10^{-6}$,}\\
    & \obj(\iter) + 10^8 \times v_{\infty}(\iter),  && \quad \text{otherwise,}
\end{empheq}
for~$\iter \in \R^n.$
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \drawperformanceprofiles{{"COBYQA","BOBYQA"}}{plain-1-50-perf-bobyqa-cobyqa-b.csv}{4}
