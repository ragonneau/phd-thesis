...
% the normal~$\nstep[1]$ should satisfy~$\nstep[1]_2 > 0$, and the tangential step~$\tstep[1]$ should satisfy~$\tstep[1]_1 = -\nstep[1]_1$ and~$\tstep[1]_2 \ge 0$.
...
It is very similar to an idea proposed earlier by Powell~\cite[Eqs.~(2.7) and~(2.8)]{Powell_1978a}.
...
Let us explain why it can be understood as a composite-step approach.
Under some assumptions (e.g., \gls{licq} at~$\iter[k]$), we can take~$\alpha \in [0, 1]$ so that the system
\begin{subequations}
...
Define the normal step~$\nstep[k]$ as the least-norm solution to this system, which then satisfies~$\norm{\nstep[k]} \le \rad[k]$, and define~$\tstep[k]$ so that it attempts to decrease~\cref{eq:sqp-subproblem-vardi-obj} within the trust region without increasing the violation of any linearized constraint.
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
If we consider the composite step~$\nstep[k] + \tstep[k]$, it then solves
\begin{subequations}
    ...
    \end{align}
\end{subequations}
which turns out to the be exactly~\cref{eq:sqp-subproblem-vardi}, as specified by the following proposition.
...
\end{subequations}
where~$\zeta \in (0, 1]$.
Further, it sets the tangential step~$\tstep[k]$ to a solution to~\cref{eq:tangential-subproblem}.
Finally, the Byrd-Omojokun composite step is~$\step[k] = \nstep[k] + \tstep[k]$.
...
% Note that this method is very close to the Vardi approach, but they are not equivalent.
...
\begin{equation*}
    \delta = \max \set{\norm{\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \nstep[k]}, \norm{\posp{\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \nstep[k]}}}.
\end{equation*}
Unlike the two former approaches, the tangential step of the \gls{cdt} approach does not attempt to maintain or decrease the violation of each constraint, but rather global violation of all the constraints simultaneously.
...
\subsubsection{A relation between the Vardi and the Byrd-Omojokun approach}

We now highlight a relation between the Vardi approach and Byrd-Omojokun approach when~$\iub = \emptyset$.
In such a case, the problem~\cref{eq:problem-cobyqa-sqp} can be reformulated as
\begin{align*}
    \min        & \quad \obj(\iter)\\
    \text{s.t.} & \quad h(\iter) = 0,\\
                & \quad \iter \in \R^n,
\end{align*}
where~$h : \R^n \to \R^m$.
Given~$\alpha \in [0, 1]$ such that~$\alpha h(\iter[k]) + \nabla h(\iter[k]) d = 0$ admits a solution whose norm is at most~$\rad[k]$, the above-mentioned normal step of the Vardi approach is
\begin{equation}
    \label{eq:vardi-equality}
    \nstep[k] = \argmin_{\step \in \R^n} \set{\norm{\step} : \alpha h(\iter[k]) + \nabla h(\iter[k]) \step = 0} = -\alpha \nabla h(\iter[k])^{\dagger} h(\iter[k]),
\end{equation}
where~$\nabla h(\iter[k])^{\dagger}$ denotes the Moore-Penrose pseudoinverse of~$\nabla h(\iter[k])$.
Moreover, the above-mentioned normal step of the Byrd-Omojokun approach solves 
\begin{subequations}
    \label{eq:byrd-omojokun-equality}
    \begin{align}
        \min        & \quad \norm{h(\iter[k]) + \nabla h(\iter[k]) \step} \label{eq:byrd-omojokun-equality-obj}\\
        \text{s.t.} & \quad \norm{\step} \le \zeta \rad[k],\\
                    & \quad \step \in \R^n,
    \end{align}
\end{subequations}
...
\begin{proposition}
    ...
    Let~$\nstep[k]$ be the normal step~\cref{eq:vardi-equality}, and let~$\step[\ast]$ be the least-norm global unconstrained minimizer of~\cref{eq:byrd-omojokun-equality-obj}.
    The step~$\nstep[k]$ solves
    \begin{equation*}
        \min_{d \in \R^n} \set{\norm{h(\iter[k]) + \nabla h(\iter[k]) \step} : \step = t \step[\ast], ~ t \ge 0, ~ \norm{d} \le \zeta \rad[k]},
    \end{equation*}
    if~$\zeta = \norm{\nstep[k]} / \rad[k]$.
\end{proposition}

\begin{proof}
    Remark that~$\step[\ast] = -\nabla h(\iter[k])^{\dagger} h(\iter[k])$, so that we only need to show that
    \begin{equation}
        \label{eq:vardi-byrd-omojokun-proof}
        \alpha = \argmin_{t \in [0, \alpha]} \norm{h(\iter[k]) - t\nabla h(\iter[k]) \nabla h(\iter[k])^{\dagger} h(\iter[k])}.
    \end{equation}
