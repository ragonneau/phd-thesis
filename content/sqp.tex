...
\begin{subequations}
    ...
    \begin{empheq}[left=\empheqlbrace]{alignat=2}
        ...
        & \con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step \le 0,        && \quad \text{otherwise,} \label{eq:vardi-ub2}
    \end{empheq}
\end{subequations}
for some~$\alpha \in [0, 1]$, where~$\act(\iter[k])$ designates the active set for the problem~\cref{eq:problem-cobyqa-sqp} at~$\iter[k]$ (see \cref{def:active-set}).
The reason for doing so is that if~$\alpha = 0$, then the trust-region subproblem is feasible, because all vectors lying in the null space of~$[\nabla \con{i}(\iter[k])]_{i \in \act(\iter[k])}^{\T}$ that are short enough satisfy the inequality constraints~\cref{eq:vardi-ub1,eq:vardi-ub2}, the equality constraints~\cref{eq:vardi-eq}, and the trust-region constraint~\cref{eq:trust-region-sqp-subproblem-tr}.
...
Given~$\alpha \in [0, 1]$ such that the system~\cref{eq:vardi} admits a solution whose norm is at most~$\rad[k]$, the normal step~$\nstep[k]$ is defined to be the least-norm solution to the system~\cref{eq:vardi} with~$\alpha = \alpha^k$, which then satisfies~$\norm{\nstep[k]} \le \rad[k]$.
...
% This composite step~$\step[k] = \nstep[k] + \tstep[k]$ solves the \gls{sqp} subproblem~\cref{eq:trust-region-sqp-subproblem} when the constraints~\cref{eq:trust-region-sqp-subproblem-ub,eq:trust-region-sqp-subproblem-eq} are replaced with~\cref{eq:vardi} for the following reason.
% Let~$\step \in \R^n$ be any point that satisfies~\cref{eq:vardi}.
% We then have
% \begin{empheq}[left=\empheqlbrace]{alignat*=2}
%     & \nabla \con{i}(\iter[k])^{\T} \step \le - \alpha \con{i}(\iter[k]) && \quad \text{if~$i \in \iub \cap \act(\iter[k])$},\\
%     & \nabla \con{i}(\iter[k])^{\T} \step = - \alpha \con{i}(\iter[k]) = \nabla \con{i}(\iter[k])^{\T} \nstep[k]    && \quad \text{if~$i \in \ieq$},
% \end{empheq}

An important remark is that, when using the Vardi approach, the required property on the merit function described in \cref{subsec:penalty-parameter-merit-function} is guaranteed.

\begin{proposition}
    If~$\step[k] = \nstep[k] + \tstep[k]$ is a composite step obtained via the Vardi approach, then
\end{proposition}
...
Note thus that, as for the Vardi approach, the required property described in \cref{subsec:penalty-parameter-merit-function} is guaranteed.
