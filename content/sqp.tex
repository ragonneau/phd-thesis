...
\begin{equation*}
    \merit[\gamma](\iter) \eqdef \obj(\iter) + \gamma \bigg( \sum_{i \in \iub} [\con{i}(\iter)]_+^p + \sum_{i \in \ieq} \abs{\con{i}(\iter)}^p \bigg)^{1/p}, \quad \text{for~$\iter \in \R^n$ and~$\gamma \ge 0$,}
\end{equation*}
...
To cope with the Maratos effect, we usually employ second-order correction steps.
The idea behind the second-order correction is to modify the current step~$\step[k] \in \R^n$ by a term~$r^k \in \R^n$, so that
\begin{empheq}[left=\empheqlbrace]{alignat*=2}
    & [\con{i}(\iter[k] + \step[k] + r^k)]_+ = \smallo(\norm{\step[k]}^2),      && i \in \iub,\\
    & \abs{\con{i}(\iter[k] + \step[k] + r^k)} = \smallo(\norm{\step[k]}^2),    && i \in \ieq.
\end{empheq}
...
Of course, such a modification should be made only when necessary.
Perhaps the simplest of these steps, inspired by~\cite{Mayne_Polak_1982}, is the least-squares solution of
\begin{align*}
    \min        & \quad \sum_{i \in \iub} [\con{i}(\iter[k] + \step[k]) + \nabla \con{i}(\iter[k] + \step[k])^{\T} r]_+^2 + \sum_{i \in \ieq} [\con{i}(\iter[k] + \step[k]) + \nabla \con{i}(\iter[k] + \step[k])^{\T} r]^2\\
    \text{s.t.} & \quad r \in \R^n.
\end{align*}
...
Another possibility to handle the Maratos effect is to use an augmented Lagrangian merit function.
Indeed, \citeauthor{Powell_Yuan_1986}~\cite[\S~4]{Powell_Yuan_1986} showed that the Maratos effect cannot occur when using the augmented Lagrangian merit function in a line-search \gls{sqp} method.
...
As we mentioned previously, since the \gls{sqp} method is a local method, we need in practice to employ a globalization strategy.
We present in what follows the trust-region framework, the new method we presented in \cref{ch:cobyqa-introduction} uses a derivative-free variation of the trust-region \gls{sqp} method.

\subsection{Overview of the method}

We present in this section the basic trust-region \gls{sqp} method.
The merit functions we consider from now on are~$\ell_2$-merit functions, defined by
\begin{equation*}
    \merit[\gamma](\iter) \eqdef \obj(\iter) + \gamma \sqrt{\sum_{i \in \iub} [\con{i}(\iter)]_+^2 + \sum_{i \in \ieq} \abs{\con{i}(\iter)}^2}, \quad \text{for~$\iter \in \R^n$ and~$\gamma \ge 0$.}
\end{equation*}
The algorithm we present below maintains a penalty parameter~$\gamma^k \ge 0$ and hence, we denote~$\merit[k]$ the function~$\merit[\gamma]$ for~$\gamma = \gamma^k$.
...
\begin{align*}
    \meritm[k](\step)   & \eqdef \nabla \obj(\iter[k])^{\T} \step + \frac{1}{2} \step^{\T} \nabla_{x, x}^2 \lag(\iter[k], \lm[k]) \step\\
                        & \qquad + \gamma^k \sqrt{\sum_{i \in \iub} [\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step]_+^2 + \sum_{i \in \ieq} [\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step]^2}, \quad \text{for~$\step \in \R^n$.}
\end{align*}
...
The basic framework is given in \cref{alg:trust-region-sqp}, where we do not explicit the second-order correction mechanism for sake of clarity.
...
When choosing the trial step~$\step[k]$, we must ensure that there exists~$\bar{\gamma} \ge 0$ such that for all~$\gamma \ge \bar{\gamma}$, the property~$\meritm[k](\step[k]) \le \meritm[k](0)$ mentioned on \cref{alg:trust-region-sqp-penalty} is well-defined.
Clearly, we must ensure that the step~$\step[k]$ satisfies either
\begin{equation*}
    \sum_{i \in \iub} [\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step[k]]_+^2 + \sum_{i \in \ieq} [\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step[k]]^2 < \sum_{i \in \iub} [\con{i}(\iter[k])]_+^2 + \sum_{i \in \ieq} \con{i}(\iter[k])^2,
\end{equation*}
or if these two term equals, then
\begin{equation*}
    \nabla \obj(\iter[k])^{\T} \step[k] + \frac{1}{2} (\step[k])^{\T} \nabla_{x, x}^2 \lag(\iter[k], \lm[k]) \step[k] \le 0.
\end{equation*}
In other word, the step~$\step[k]$ must either improve the infeasibility of~$\iter[k]$ with respect to the linearized constraints, or decrease the objective function of the \gls{sqp} subproblem without worsening the possible infeasibility.
...
\citeauthor{Powell_1994} used in his solver \gls{cobyla}~\cite{Powell_1994} a very similar update of the penalty parameter, with the values~$\eta_3 = 3/2$ and~$\eta_4 = 2$.
...
\begin{align*}
    \min        & \quad \sum_{i \in \iub} [\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step]_+^2 + \sum_{i \in \ieq} [\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step]^2\\
...
An important remark is that, when using the Vardi approach, the required property on the merit function in \cref{alg:trust-region-sqp-penalty} of \cref{alg:trust-region-sqp} is true.

\subsubsection{Byrd-Omojokun approach}

\todo[noline]{Compare with the Vardi approach.}
...
However, as for the Vardi approach, the required property on the merit function in \cref{alg:trust-region-sqp-penalty} of \cref{alg:trust-region-sqp} is true.
...
\begin{empheq}[left=\empheqlbrace]{alignat*=2}
    ...
    & \norm{[\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step]_+} \le \delta,    && \quad \text{if~$i \in \iub$,}
\end{empheq}
