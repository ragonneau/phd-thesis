...
\end{equation*}
The algorithm we present below maintains a penalty parameter~$\gamma^k \ge 0$ and hence, we denote~$\merit[k]$ the function~$\merit[\gamma^k]$ for simplicity.
...
\begin{align*}
    \meritm[k](\step)   & \eqdef \nabla \obj(\iter[k])^{\T} \step + \frac{1}{2} \step^{\T} \nabla_{x, x}^2 \lag(\iter[k], \lm[k]) \step\\
                        & \qquad + \gamma^k \sqrt{\sum_{i \in \iub} [\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step]_+^2 + \sum_{i \in \ieq} \abs{\con{i}(\iter[k]) + \nabla \con{i}(\iter[k])^{\T} \step}^2},\\
                        & \qquad \text{for~$\step \in \R^n$ and~$\gamma \ge 0$.}
\end{align*}
As mentioned by~\cite[\S~15.3.2.1]{Conn_Gould_Toint_2000}, such a function is a second-order approximation of~$\merit[k]$ at~$\iter[k]$, which is suitable for use in trust-region \gls{sqp} method.
