...
Inspired by an observation in~\cite[\S~2]{Gill_Wong_2011}, we can also interpret the objective function of the \gls{sqp} subproblem as an approximation of the original objective function in the tangent space of the feasible set.
As shown in \cref{thm:sqp-path}, when approximating~$\obj$ in such a space, we will naturally get the Hessian matrix of the Lagrangian in the second-order term\todo{If there is a new theorem, this needs to be revised}.
...
\Cref{thm:sqp-path} characterizes this approximation of~$\obj$ when~$\bar{\iter}$ is feasible.
...
\begin{theorem}
    ...
    If~$\iter$ is twice differentiable and~$\iter''$ is locally Lipschitz continuous for~$t \ge 0$, then there exist constants~$\nu \ge 0$ and~$\epsilon > 0$ such that
    \begin{equation*}
...
\begin{proof}
    ...
    According to~\cref{eq:sqp-path-proof-1}, only the second-order terms of~$\widehat{\phi}(t)$ and~$Q(\iter'(0) t)$ differ, so that
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    where the second equality uses the fact that~$\iter(0) = \bar{\iter}$.
...
The basic framework is given in \cref{alg:trust-region-sqp}, where we do not explicitize the second-order correction mechanism for simplicity.
...
Therefore, the normal step~$\nstep[k]$ of the Vardi approach is a truncated Newton step for~\cref{eq:byrd-omojokun-original} by setting~$\zeta = \norm{\nstep[k]} / \rad[k]  = \alpha \norm{\step[\ast]} / \rad[k]$.
