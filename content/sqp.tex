...
\todo[noline]{Reformulate using simple notations}

\todo[noline]{What is there is inequality constraints (maybe slack variables)}

\begin{proposition}
    Assume that~$\iub = \emptyset$, and denote~$Q(\step)$ the objective function~\cref{eq:byrd-omojokun-obj}.
    Let~$\nstep[k]$ be the least-norm solution to~\cref{eq:vardi-normal-step}, and let~$\step[\ast]$ be the least-norm global minimizer of~$Q$.
    If~$\zeta = \norm{\nstep[k]} / \rad[k]$, then
    \begin{equation*}
        \nstep[k] = \argmin \set{Q(\step) : \step = t \step[\ast], ~ t \ge 0, ~ \norm{d} \le \rad[k]}
    \end{equation*}
    % ~$\nstep[k]$ minimizes~\cref{eq:byrd-omojokun-obj} along the direction~$\step[\ast]$ such that~$\norm{\nstep[k]} \le \zeta \rad[k]$.
\end{proposition}
