...
\begin{algorithm}
    \caption{Basic trust-region \glsxtrshort{sqp} method}
...
\subsection{Other methods for solving the trust-region \glsfmtshort{sqp} subproblem}
...
\begin{subequations}
    \label{eq:slpqp}
    \begin{align}
        \min        & \quad \nabla \obj(\iter[k])^{\T} \step + \frac{1}{2} \step^{\T} \nabla_{x, x}^2 \lag(\iter[k], \lm[k]) \step + \Phi_p(d)\\
        \text{s.t.} & \quad \norm{d} \le \rad[k]
    \end{align}
\end{subequations}
...
\end{equation*}
% In particular, \citeauthor{Yuan_1995}~\cite{Yuan_1995} proposed it with~$p = \infty$.
...
\todo[noline]{They did not propose it to solve a trust-region SQP subproblem, but as penalty methods}
...
For example, \gls{snopt}~\cite{Gill_Murray_Saunders_2002,Gill_Murray_Saunders_2005} is an implementation of the \gls{sqp} method for inequality-constrained optimization, which approximates the Hessians of the Lagrangian function with a limited-memory quasi-Newton method\todo{Put citations}.
...
\begin{itemize}
    \item NLPQLP (cite \url{http://klaus-schittkowski.de/NLPQLP.pdf}).
    \item \cite{Lalee_Nocedal_Plantenga_1998}.
\end{itemize}
