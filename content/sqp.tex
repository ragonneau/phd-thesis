...
We first present the general \gls{sqp} framework in \cref{sec:sqp-method}, and we interpret it from different angles\todo{Finish the introduction and the summary}.
...
\section{Existing \glsfmtshort{sqp} software}

\begin{itemize}
    \item SNOPT, CONDOR, S$\ell_1$QP, SLSQP (merit functions, inequality constraints, infeasibility).
\end{itemize}

\section{Concluding remarks}

\begin{itemize}
    \item Brief summary, highlighting our contribution.
    \item Some further discussions (relation between Vardi and Byrd-Omojokun in the general case, the relation between the augmented Lagrangian and the \gls{sqp} method (both theory and computations)).
    \item We need to make experiments to show that the way to handle the inequality constraints in the composite-step approach is good.
\end{itemize}
