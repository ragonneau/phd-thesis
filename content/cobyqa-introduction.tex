...
\todo[noline]{Why algorithmfootnote does not work}
...
Therefore, given the iterate~$\iter[k + 1]$, it is natural to set the estimated Lagrange multiplier~$\lm[k + 1]$ to the one that attempt to satisfy the \gls{kkt} conditions as much as possible using the information available so far.
...
\begin{subequations}
    ...
    \begin{align}
        ...
        \text{s.t.} & \quad \lm_i \conm[k]{i}(\iter[k + 1]) = 0, ~ i \in \iub \label{eq:least-squares-lagrange-multipliers-cobyqa-complementary-slackness}\\
