...
The first reference of such a method appears to be in the Ph.D. thesis of \citeauthor{Wilson_1963}~\cite{Wilson_1963}.
The \gls{sqp} method was later modified using quasi-Newton approximations by \citeauthor{Garcia-Palomares_Mangasarian_1976}~\cite{Garcia-Palomares_Mangasarian_1976}.
The theoretical properties of the \gls{sqp} method were thus shown by \citeauthor{Han_1976}~\cite{Han_1976,Han_1977} and \citeauthor{Powell_1978a}~\cite{Powell_1978a,Powell_1978b,Powell_1978c}.
The \gls{sqp} method is presented in \cref{alg:sqp}.
...
\begin{algorithm}[ht]
    ...
    \For{$k = 0, 1, \dots$}{
        Define~$H^k \approx \nabla_{x, x}^2 \lag(\iter[k], \lambda^k)$ for some Lagrange multiplier~$\lambda^k = [\lambda_i^k]_{i \in \iub \cup \ieq}^{\T}$ \nllabel{alg:sqp-hessian}\;
        ...
        \begin{subequations}
            ...
            \end{algomathalign}
        \end{subequations}
        Set~$\iter[k + 1] \gets \iter[k] + \step[k]$\;
    }
\end{algorithm}

The matrix~$H^k$ in \cref{alg:sqp-hessian} is usually either the exact Hessian, or a quasi-Newton approximation if~$\nabla_{x, x}^2 \lag(\iter[k], \lambda^k)$ is expensive.
...
Moreover, if~$H^k = \nabla_{x, x}^2 \lag(\iter[k], \lambda^k)$ and if the Lagrange multiplier~$\lambda^k$ is close enough from a true Lagrange multiplier at the solution, then the convergence is Q-quadratic.
...
According to \cref{thm:first-order-necessary-conditions}, if~$\iter[\ast] \in \R^n$ is a local solution to the problem~\cref{eq:problem-cobyqa-sqp}, under some mild assumptions, there exists a Lagrange multiplier~$\lambda^{\ast} = [\lambda_i^{\ast}]_{i \in \iub \cup \ieq}^{\T}$ with~$\lambda_i^{\ast} \in \R$ for all~$i \in \iub \cup \ieq$ such that
\begin{subequations}
    \label{eq:sqp-kkt}
    ...
    \end{empheq}
\end{subequations}
Let~$(\iter[k], \lambda^k)$ be some approximation of~$(\iter[\ast], \lambda^{\ast})$, and let~$(\step, u^k)$ satisfy
\begin{subequations}
    \label{eq:sqp-kkt-step}
    \begin{empheq}[left=\empheqlbrace]{alignat=2}
        & \nabla_x \lag(\iter[k], \lambda^k + u^k) + \nabla_{x, x}^2 \lag(\iter[k], \lambda^k) \step = 0,   && \\
    ...
    \end{empheq}
\end{subequations}
Note that this conditions resemble closely the first-order Taylor approximation at~$(\iter[k], \lambda^k)$ of~\cref{eq:sqp-kkt}, in which case~$(\step, u^k)$ would be a Newton-Raphson step.
...
We thus remark that the conditions~\cref{eq:sqp-kkt-step} are nothing but the \gls{kkt} conditions of the \gls{sqp} subproblem~\cref{eq:sqp-subproblem} associated with the Lagrange multiplier~$\lambda^k + u^k$ with~$H^k = \nabla_{x, x}^2 \lag(\iter[k], \lambda^k)$.
...
\end{equation*}
The \gls{sqp} subproblem~\cref{eq:sqp-subproblem} with~$H^k = \nabla_{x, x}^2 \lag(\iter[k], \lambda^k)$ can then be seen as the minimization of the second-order Taylor approximation of~$\lag[k]$ subject to the linearized constraints, i.e.,
\begin{align}
...
\begin{align*}
    \lag[\mathsf{A}](\iter[k] + \step, s^k, \lambda^k)  & \approx \obj(\iter[k]) + \nabla \obj(\iter[k])^{\T} \step + \frac{1}{2} \step^{\T} \nabla^2 \obj(\iter[k]) \step\\
                                                        ...
                                                        & \approx \obj(\iter[k]) + \nabla \obj(\iter[k])^{\T} \step + \frac{1}{2} \step^{\T} \nabla_{x, x}^2 \lag(\iter[k], \lambda^k) \step\\
...
\begin{equation*}
    \tilde{H}^k(\lambda) \eqdef \nabla \obj(\iter[k]) + \sum_{i \in \iub} [\lambda_i + \gamma (\con{i}(\iter[k]) + s_i)] \nabla^2 \con{i}(\iter[k]), \quad \text{for~$\lambda_i \in \R$, with~$i \in \iub$.}
\end{equation*}
This approximation of~$\lag[\mathsf{A}]$ is the augmented Lagrangian of the \gls{sqp} subproblem~\cref{eq:sqp-subproblem} with~$H^k = \tilde{H}^k(\lambda^k)$.
...
The difficulty of maintaining an appropriate geometry of the interpolation set is discussed in \cref{subsec:geometry-interpolation-set}, and requires to modify \cref{alg:derivative-free-sqp}.
...
\begin{algorithm}[ht]
    ...
    \For{$k = 0, 1, \dots$}{
        ...
        \end{algomathdisplay}
        \eIf{$\ratio[k] \ge 0$}{
            Update the current step~$\iter[k + 1] \gets \iter[k] + \step[k]$\;
        }{
            Retain the current step~$\iter[k + 1] \gets \iter[k]$\;
        }
        Replace a point in~$\xpt[k]$ by~$\iter[k + 1]$ to obtain~$\xpt[k + 1]$ \nllabel{alg:derivative-free-sqp-set}\;
...
A solution to this subproblem provides a trial step~$\step \in \R^k$.
Further, this trial step is either accepted or not, i.e.,~$\iter[k + 1]$ is either set to~$\iter[k] + \step$ or to~$\iter[k]$, depending on the value of the trust-region ratio.
...
\begin{equation*}
    \ratio[k] \eqdef \frac{\merit[k](\iter[k]) - \merit[k](\iter[k] + \step)}{\meritm[k](\iter[k]) - \meritm[k](\iter[k] + \step)}.
\end{equation*}
...
\begin{algorithm}[ht]
    ...
    \DontPrintSemicolon
    \KwData{Current lower bound on the trust-region radius~$\radlb[k] > 0$, current trust-region radius~$\rad[k] > 0$, current trust-region ratio~$\ratio[k] \in \R$, current trial step~$\step \in \R^n$, and parameters~$0 < \eta_1 \le \eta_2 < 1 \le \eta_3$ and~$0 < \theta_1 < 1 < \theta_2$}
...
Note that the general inequality constraints~\cref{eq:problem-cobyqa-sqp-ub} may include bound constraints.
...
\label{subsec:geometry-interpolation-set}
