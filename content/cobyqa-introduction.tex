...
Inspired by the performance of \gls{uobyqa}, \gls{newuoa}, \gls{bobyqa}, and \gls{lincoa} (see \cref{subsec:uobyqa,subsec:newuoa-bobyqa-lincoa}), we employ the following paradigm for managing the trust-region radius in \gls{cobyqa}, proposed by Powell~\cite{Powell_2002,Powell_2006,Powell_2009}, in place of \cref{alg:derivative-free-trust-region-sqp-radius} of \cref{alg:derivative-free-trust-region-sqp}.
...
The parameters chosen in \gls{cobyqa} are~$\eta_1 = 0.1$,~$\eta_2 = 0.7$,~$\theta_1 = 0.5$,~$\theta_2 = \sqrt{2}$,~$\theta_3 = 2$, and~$\theta_4 = 1.5$.
...
\begin{algorithm}
    ...
    \DontPrintSemicolon
    \onehalfspacing
    \KwData{Current lower bound on the trust-region radius~$\radlb[k] > 0$, current trust-region radius~$\rad[k] \ge \radlb[k]$, current trust-region ratio~$\ratio[k] \in \R$, current trial step~$\step[k] \in \R^n$, and parameters~$0 < \eta_1 \le \eta_2 < 1$,~$0 < \theta_1 < 1 < \theta_2 < \theta_3$, and~$\theta_4 \ge 1$.}
    \KwResult{Updated trust-region radius~$\rad[k + 1]$.}
    Update the trust-region radius
    \begin{algoempheq}[left={\rad[k + 1] \gets \empheqlbrace}]{alignat*=2}
        & \theta_1 \rad[k]                                                                  && \quad \text{if~$\ratio[k] \le \eta_1$,}\\
        & \min \set{\theta_1 \rad[k], \norm{\step[k]}}                                      && \quad \text{if~$\eta_1 < \ratio[k] \le \eta_2$,}\\
        & \min \set{\theta_2 \rad[k], \max \set{\theta_1 \rad[k], \theta_3 \norm{\step}}}   && \quad \text{otherwise}
    \end{algoempheq}
    \If{$\rad[k + 1] \le \theta_4 \radlb[k]$}{
...
It is identical to the one in \gls{uobyqa}~\cite{Powell_2002}, \gls{newuoa}~\cite{Powell_2006}, \gls{bobyqa}~\cite{Powell_2009}, and \gls{lincoa}.
...
If \cref{alg:reducing-lower-bound-trust-region-radius-stop} of \cref{alg:reducing-lower-bound-trust-region-radius} is reached, we consider that the optimization method should stop.
This is in fact the definition of \enquote{convergence} we mentioned in \cref{alg:derivative-free-trust-region-sqp}, which is the stopping criterion used in the Powell's solvers.
...
However, if this process has been trigerred several times consecutively, then the method decreases~$\radlb[k + 1]$ before generating a new trust-region step.
