...
If~$\nabla_{x, x}^2 \lag(\iter[k], \lambda^k)$ is unavailable or expensive, it can be replaced with an approximation (e.g., a quasi-Newton approximation\todo{Who did it?}).
Under some mild assumptions on the regularity of the objective and constraint functions, the \gls{sqp} method is locally Q-superlinearly convergent\todo{Who did it? Say that it is also quadratic}.
