...
\begin{algorithm}
    ...
    \DontPrintSemicolon
    \onehalfspacing
    \algorithmfootnote{\textsuperscript{$\dagger$}Recall that supercripts are iteration counters rather than exponents.}
...
The condition~\cref{eq:least-squares-lagrange-multipliers-cobyqa-complementary-slackness} attempts to satisfy the complementary slackness conditions on the inequality constraints that are satisfied.
We do not take into account the complementary slackness conditions on the inequality constraints that are not satisfied, because these constraints are likely to be active at the solution\todo{Improve the two previous sentence}.
