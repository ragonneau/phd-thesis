...
Let us first consider the case~$\rho^k \ge 0$.
We want to replace a point from~$\xpt[k]$ by~$\iter[k] + \step[k]$ to obtain~$\xpt[k + 1]$.
We denote~$\bar{\iter}$ the closest point from~$\iter[k]$ that solves
\begin{equation*}
...
Recall from \cref{subsec:implementation-symmetric-broyden-update} that the inverse of the coefficient matrix of the interpolation system is maintained.
...
Coping with the case~$\rho^k < 0$ is now clear.
...
Hence, we do not employ this technique.
...
% During a trust-region iteration, a trial point is generated, and a point
...
\todo[noline]{Present the mechanism}
