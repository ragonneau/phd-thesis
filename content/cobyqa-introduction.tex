...
\todo[noline]{Revise the assumptions to make~$\phi''$ Lipschitz continuous}

\begin{theorem}
    Assume that~$\obj$ is twice differentiable with Lipschitz-continuous Hessian matrices, that~$h$ is twice differentiable.
    Let~$\iter(t)$ be a twice differentiable feasible path starting at~$\iter[k]$ and parametrized by a nonnegative scalar~$t$, i.e.,~$h(\iter(t)) = 0$ and~$\iter(t) \ge 0$ for~$t \ge 0$, and~$\iter(0) = \iter[k]$, with Lipschitz-continuous second derivative.
    If~$H^k = \nabla_{x, x}^2 \lag(\iter[k], \lm[k])$, then there exist constants~$\nu_1 \ge 0$ and~$\nu_2 \ge 0$ such that for all~$t \ge 0$,
    \begin{equation*}
        \abs{\obj(\iter(t)) - Q_k(x'(0) t)} \le (\nu_1 t + \nu_2) t^2.
    \end{equation*}
    Moreover, if~$\iter[k]$ is a local solution to~\cref{eq:problem-cobyqa-auglag}, then we can set~$\nu_2 = 0$.
\end{theorem}

\begin{proof}
    Let~$\phi$ be the function defined by~$\phi(t) = \obj(x(t))$ for~$t \ge 0$.
    For any~$t \ge 0$, we have
    \begin{empheq}[left=\empheqlbrace]{alignat*=1}
        & \phi'(t) = \iter'(t)^{\T} \nabla \obj(\iter(t)),\\
        & \phi''(t) = \iter''(t)^{\T} \nabla \obj(\iter(t)) + \iter'(t)^{\T} \nabla^2 \obj(\iter(t)) \iter'(t).
    \end{empheq}
    Let~$\widehat{\phi}$ be the second-order Taylor expansion of~$\phi$ at~$0$.
    % \begin{equation*}
    %     \widehat{\phi}(t) \eqdef \phi(0) + t \phi'(0) + \frac{t^2}{2} \phi''(0), \quad \text{for~$t \ge 0$.}
    % \end{equation*}
    We then have
    \begin{equation*}
        \abs{\obj(\iter(t)) - Q_k(\iter'(0) t)} \le \abs{\phi(t) - \widehat{\phi}(t)} + \abs{\widehat{\phi}(t) - Q_k(\iter'(0) t)}.
    \end{equation*}
    By assumption, there exists a constant~$\nu_1 \ge 0$ such that~$\abs{\phi(t) - \widehat{\phi}(t)} \le \nu_1 t^3$.
    Moreover, since~$h(\iter(t)) = 0$ for all~$t \ge 0$, the derivatives of its components are zero and hence,
    \begin{equation*}
        \iter''(0)^{\T} \nabla h_i(\iter[k]) + \iter'(0) \nabla^2 h_i(\iter[k]) \iter'(0) = 0, \quad \text{for~$i \in \set{1, 2, \dots, m}$.}
    \end{equation*}
    Therefore, we have
    \begin{align*}
        \abs{\widehat{\phi}(t) - Q_k(x'(0) t)}  & = \frac{t^2}{2} \abs[\bigg]{\iter''(0)^{\T} \nabla \obj(\iter[k]) - \sum_{i = 1}^m \lm[k]_i \iter'(0) \nabla^2 h_i(\iter[k]) \iter'(0)}\\
                                                & \le \frac{t^2}{2} \abs{\iter''(0)^{\T} [\nabla \obj(\iter[k]) + \nabla h(x^k)^{\T} \lm[k]]}
    \end{align*}
    By setting~$\nu_2 = \abs{\iter''(0)^{\T} [\nabla \obj(\iter[k]) + \nabla h(x^k)^{\T} \lm[k]]} / 2$, we thus have
    \begin{equation*}
        \abs{\obj(\iter(t)) - Q_k(\iter'(0) t)} \le \nu_1 t^3 + \nu_2 t^2 = (\nu_1 t + \nu_2) t^2.
    \end{equation*}
\end{proof}
...
\begin{proof}
    By direct calculations, we remark that
    \begin{equation*}
