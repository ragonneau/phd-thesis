...
The \gls{sqp} method was also introduced by \citeauthor{Biggs_1975}~\cite{Biggs_1975}, and later modified using quasi-Newton approximations by \citeauthor{Garcia-Palomares_Mangasarian_1976}~\cite{Garcia-Palomares_Mangasarian_1976}.
...
\begin{algorithm}
    ...
    \For{$k = 0, 1, \dots$}{
        Define~$H^k \approx \nabla_{x, x}^2 \lag(\iter[k], \lm[k])$ for some Lagrange multiplier~$\lm[k] = [\lm[k]_i]_{i \in \iub \cup \ieq}^{\T}$ \nllabel{alg:sqp-hessian}\;
...
Moreover, if~$H^k = \nabla_{x, x}^2 \lag(\iter[k], \lm[k])$ and if the Lagrange multiplier~$\lm[k]$ is close enough from a true Lagrange multiplier at the solution towards which~$\set{\iter[k]}_{k \ge 0}$ converges, then the convergence is Q-quadratic.
...
\begin{subequations}
    ...
    \end{empheq}
\end{subequations}
Let~$(\iter[k], \lm[k])$ be some approximation of~$(\iter[\ast], \lm[\ast])$, and let~$(\step, u^k)$ satisfy
\begin{subequations}
...
\begin{algorithm}
    ...
    \For{$k = 0, 1, \dots$}{
        ...
        Update the current step~$\iter[k + 1] \gets \argmin \set{\merit[k](y)}_{y \in \xpt[k + 1]}$\;
...
Moreover, note that~$\iter[k + 1]$ necessarily contains~$\iter[k] + \step[k]$, and also contain~$\iter[k]$ if~$\ratio[k] < 0$.
Thus, our adaptation of the trust-region framework for unconstrained optimization is reasonable.

\subsection{Updating the trust-region radius}
