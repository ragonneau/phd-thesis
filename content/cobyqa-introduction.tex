...
Note that the approximation error in \cref{thm:sqp-path} depends on the magnitude of~$\nabla \obj(\bar{\iter}) + \nabla h(\bar{\iter})^{\T} \bar{\lm}$.
...
Intriguingly, a traditional augmented Lagrangian method for solving the problem~\cref{eq:problem-cobyqa-auglag} would update the dual variable~$\lm[k]$ by
\begin{equation*}
