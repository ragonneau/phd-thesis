...
Given a perturbation~$\epsilon \ge 0$ and an point~$\iter[k] = [1 + \epsilon, 0]^{\T}$, if the second-order term in~\cref{eq:sqp-subproblem-obj} included only~$\nabla^2 \obj(\iter[k])$, the \gls{sqp} subproblem would be
\begin{subequations}
    \begin{align}
...
% In addition, it seems to indicate that a large modification of the second component of~$\iter[k]$ must be made, although~$\iter[k]_2 = \iter[\ast]_2$.
