...
\begin{algorithm}
    \caption{Bound-constrained \glsfmtshort{tcg} method~\cite[\S~3]{Powell_2009}}
    \label{alg:bvtcg}
    \DontPrintSemicolon
    \onehalfspacing
    ...
    \For{$k = 0, 1, \dots$ until~$\norm{\pstep[k]} = 0$}{
        ...
        \uElseIf{$\alpha^k = \alpha_B^k$}{
            ...
            Restart the \gls{tcg} iteration by setting~$\pstep[k + 1] \gets -\Pi^{k + 1}(\nabla Q(\sstep[k + 1]))$\;
        }
        \Else{
...
Therefore, the initial working set~$\mathcal{W}^0$ is chosen to be~$\set{j \in \mathcal{J}(\sstep[0]) : e_j^{\T} A \sstep[0] = 0}$.
% (or a subset of it, so that~$\set{e_j^{\T} A : j \in \mathcal{W}^0}$ is a basis of~$\vspan \set{e_j^{\T} A : j \in \mathcal{J}(\sstep[0]), ~ e_j^{\T} A \sstep[0] = 0}$).
...
\begin{algorithm}
    \caption{Linearly-constrained \glsfmtshort{tcg} method~\cite[\S~3]{Powell_2015}}
    \label{alg:lctcg}
    \DontPrintSemicolon
    \onehalfspacing
    \KwData{Matrices~$A$ and~$C$, right-hand side~$b \ge 0$, trust-region radius~$\rad > 0$, and constant~$\sigma \in (0, 1)$.}
    ...
    \For{$k = 0, 1, \dots$ until~$\norm{\pstep[k]} = 0$}{
        ...
        \uElseIf{$\alpha^k = \alpha_L^k$ and~$\norm{\sstep[k + 1]} \le (1 - \sigma) \rad$}{
            Update~$\hat{k} \gets k + 1$\;
            Restart the \gls{tcg} iteration by setting~$\pstep[k + 1] \gets -\Pi^{\hat{k}} \big( \nabla Q(\sstep[k + 1]) \big)$\; \nllabel{alg:lctcg-sd-restart}
        }
        \Else{
            Set
...
% It is important to note that the method stops if a new constraint is hit at a point close from the trust-region boundary.
...
Moreover, similarly to \cref{alg:lctcg}, after calculating the search directions at \cref{alg:lctcg-sd-init,alg:lctcg-sd-restart}\todo{The number are not correct}, the error term
\begin{equation*}
