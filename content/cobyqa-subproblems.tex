...
\begin{algorithm}
    ...
    \For{$k = 0, 1, \dots$ until~$\norm{\pstep[k]} = 0$\label{alg:tcg-stop}}{
        Set
...
It is also worth mentioning that the stopping criterion~$\norm{\pstep[k]} = 0$ in \cref{alg:tcg-stop} of \cref{alg:tcg} is equivalent to
\begin{equation*}
    \nabla Q(\sstep[k])^{\T} \pstep[k] \ge 0.
\end{equation*}
It is obvious that~$\norm{\pstep[k]} = 0$ implies~$\nabla Q(\sstep[k])^{\T} \pstep[k] \ge 0$.
For the converse, we assume without loss of generality that~$k \ge 1$.
Since~$\nabla Q(\sstep[k])^{\T} \pstep[k - 1] = 0$, we can see that~$\nabla Q(\sstep[k])^{\T} \pstep[k] \ge
0$ implies~$\nabla Q(\sstep[k]) = 0$ due to the definition of~$\pstep[k]$, and hence~$\beta^{k - 1} = 0$ so that~$\pstep[k] = 0$.
...
\begin{algorithm}
    \caption[Linearly-constrained \glsfmtshort{tcg} method]{Linearly-constrained \glsfmtshort{tcg}
    method~\cite[\S~3, \S-5]{Powell_2015}}
    \label{alg:lctcg}
    \DontPrintSemicolon
    \onehalfspacing
