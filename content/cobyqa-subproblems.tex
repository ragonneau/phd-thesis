...
In practice,~$\alpha_Q^k$ in \cref{alg:tcg-alphaq} of \cref{alg:tcg} is calculated by
\begin{subequations}
    \label{eq:tcg-alphaq}
...
% Moreover, we observe the following, which is useful for a robust implementation.
...
The following simple observation is taken from~\cite[\S~3]{Powell_2015}.
...
\begin{proof}
    ...
    \begin{subequations}
        ...
        \end{align}
    \end{subequations}
    % There exists a neighborhood of~$\tilde{\pstep}$ such that~$\tilde{\pstep}$ is the least value of the objective function in the intersection of this neighborhood with the feasible region.
    % Moreover, this neighborhood can be small enough to not intersect the boundary of the inactive constraints.
    % But the intersection of this neighborhood with the feasible set of the first problem is the intersection of this neighborhood with the feasible set of the second one.
