...
Rather, we only need to find a solution~$\sstep[\ast]$ that satisfies
\begin{equation*}
...
\begin{subequations}
    ...
    \begin{align}
        \min        & \quad Q(\sstep) \label{eq:problem-bvtcg-obj}\\
...
\begin{equation}
    ...
    \mathcal{W}^0 \eqdef \set[\bigg]{i \in \set{1, 2, \dots, n} : \xl_i = 0 ~ \text{and} ~ \frac{\partial Q}{\partial \sstep_i}(0) \ge 0, ~ \text{or} ~ \xu_i = 0 ~ \text{and} ~ \frac{\partial Q}{\partial \sstep_i}(0) \le 0}.
\end{equation}
The complete framework is described in \cref{alg:bvtcg}, where~$\Pi^k(z)$ is denotes the vector whose~$i$th component is~$z_i$ if~$i \notin \mathcal{W}^k$, and zero otherwise.
...
\begin{align*}
    ...
    \text{s.t.} & \quad \xl \le \sstep \le \xu,\\
                & \quad \norm{\sstep} \le \rad,\\
                & \quad \sstep \in \vspan @@ \set{\Pi^{\ast}(\sstep[\ast]), \Pi^{\ast}(\nabla Q(\sstep[\ast]))} \subseteq \R^n,
\end{align*}
where~$\Pi^{\ast}$ denotes the function~$\Pi^k$ at the last iteration.
Note that a substantial reduction in~$Q$ is possible only if both~$\Pi^{\ast}(\nabla Q(\sstep[\ast]))$ and the angle between~$\Pi^{\ast}(\sstep[\ast])$ and~$-\Pi^{\ast}(\nabla Q(\sstep[\ast]))$ are important.
The method designed by \citeauthor{Powell_2009} attempts to move round the trust-region boundary only if
\begin{equation*}
    \norm{\Pi^{\ast}(\sstep[\ast])}^2 \norm{\Pi^{\ast}(\nabla Q(\sstep[\ast]))}^2 - [\Pi^{\ast}(\sstep[\ast])^{\T} \Pi^{\ast}(\nabla Q(\sstep[\ast]))]^2 \le \xi [Q(0) - Q(\sstep[\ast])]^2,
\end{equation*}
for some~$\xi \in (0, 1)$.
In \gls{cobyqa}, we have~$\xi = 10^{-4}$.
When the amelioration mechanism is entertain, it builds an orthogonal basis~$\set{\Pi^{\ast}(\sstep[\ast]), w}$ of~$\vspan \set{\Pi^{\ast}(\sstep[\ast]), \Pi^{\ast}(\nabla Q(\sstep[\ast]))}$ by computing the vector~$w \in \R^n$ that satisfy
\begin{empheq}[left=\empheqlbrace]{alignat*=1}
    & w^{\T} \Pi^{\ast}(\sstep[\ast]) = 0\\
    & w^{\T} \Pi^{\ast}(\nabla Q(\sstep[\ast])) < 0\\
    & \norm{w} = \norm{\Pi^{\ast}(\sstep[\ast])}.
\end{empheq}
Further, the method considers the function
\begin{equation*}
    \sstep(\theta) \eqdef \sstep[\ast] + (\cos \theta - 1) \Pi^{\ast}(\sstep[\ast]) + \sin \theta w, \quad \text{for~$0 \le \theta \le \pi / 4$,}
\end{equation*}
and solve
\begin{align*}
    \min        & \quad Q(\sstep(\theta))\\
    \text{s.t.} & \quad \xl \le \sstep(\theta) \le \xu,\\
...
\index{TCG@\glsfmtshort{tcg}!linearly-constrained|(}We now present the linearly-constrained variation of the \gls{tcg} method designed by \citeauthor{Powell_2015} for his solver \gls{lincoa}~\cite{Powell_2015}.
...
More precisely, for some feasible~$\sstep \in \R^n$, we let
\begin{equation*}
...
\begin{subequations}
    ...
    \begin{align}
        \min        & \quad \frac{1}{2} \norm{\sstep - z}^2\\
        \text{s.t.} & \quad e_j^{\T} A \sstep, ~ j \in \mathcal{J}(\sstep[k]) \le 0,\\
                    & \quad C \sstep = 0,\\
                    & \quad \sstep \in \R^n. \nonumber
    \end{align}
\end{subequations}
...
\begin{algorithm}
    ...
    Set the search direction~$\pstep[0] \gets -\Pi^{\hat{k}} \big( \nabla Q(\sstep[0]) \big)$\; \nllabel{alg:lctcg-sd-init}
    \For{$k = 0, 1, \dots$ until~$\norm{\pstep[k]} = 0$}{
        Set
        \begin{algoempheq}[left={\alpha_Q^k \gets \empheqlbrace}]{alignat*=2}
            & \frac{- \nabla Q(\sstep[k])^{\T} \pstep[k]}{(\pstep[k])^{\T} (\nabla^2 Q) \pstep[k]}  && \quad \text{if~$(\pstep[k])^{\T} (\nabla^2 Q) \pstep[k] > 0$,}\\
        ...
        \uIf{$\alpha^k = \alpha_L^k$ and~$\norm{\sstep[k + 1]} \le (1 - \sigma) \rad$}{
            Update~$\hat{k} = k + 1$\;
            Set~$\pstep[k + 1] \gets -\Pi^{\hat{k}} \big( \nabla Q(\sstep[k + 1]) \big)$\; \nllabel{alg:lctcg-sd-restart}
        }
...
The linearly-constrained \gls{tcg} procedure with these additional stopping criteria is implemented in \gls{cobyqa} under the function \texttt{cobyqa.linalg.lctcg}.\index{TCG@\glsfmtshort{tcg}!linearly-constrained|)}
...
\begin{subequations}
    ...
    \begin{align}
        \min        & \quad Q(\sstep, v) \eqdef \frac{1}{2} \big[ \norm{v}^2 + \norm{C \sstep - d}^2 \big]\\
        \text{s.t.} & \quad E \sstep + F v \le h,\\
                    & \quad \norm{\sstep} \le \rad,\\
                    & \quad \sstep \in \R^n, ~ v \in \R^{m_1}, \nonumber
    \end{align}
\end{subequations}
where
\begin{equation*}
...
\begin{subequations}
    ...
    \begin{align}
        \min        & \quad \mu(\sstep) \eqdef \frac{1}{2} \norm{A \sstep - b}^2\\
...
\begin{align*}
    ...
    \text{s.t.} & \quad \xl \le \sstep \le \xu,\\
                & \quad \norm{\sstep} \le \rad,\\
                & \quad \sstep \in \set{\alpha y : \alpha \in (0, 1), ~ y \in \xpt} \subseteq \R^n.
\end{align*}
