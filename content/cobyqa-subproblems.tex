...
\begin{algorithm}
    ...
    \For{$k = 0, 1, \dots$ until~$\norm{\pstep[k]} = 0$}{
        ...
        \end{algoempheq} \label{alg:tcg-alphaq}
...
Remark that~$\alpha_Q^k$ in \cref{alg:tcg-alphaq} of \cref{alg:tcg} is in fact
\begin{equation*}
    \argmin_{\alpha \ge 0} Q(\sstep[k] + \alpha \pstep[k]),
\end{equation*}
where we define this~$\argmin$ to~$\infty$ if~$Q$ is concave along~$\set{\sstep[k] + \alpha \pstep[k] : \alpha \ge 0}$.
It is also worth mentioning that the stopping criteria~$\norm{\pstep[k]} = 0$ is equivalent to
\begin{equation*}
    \nabla Q(\sstep[k])^{\T} \pstep[k] \ge 0.
\end{equation*}
Indeed, assume that~$\nabla Q(\sstep[k])^{\T} \pstep[k] \ge 0$.
Since~$Q(\sstep[k])^{\T} \pstep[k - 1] = 0$, we have~$\nabla Q(\sstep[k]) = 0$ by definition of~$\pstep[k]$.
Hence,~$\beta^{k - 1} = 0$ so that~$\pstep[k] = 0$.
...
Note that we define~$\alpha_B^k$ in \cref{alg:bvtcg-alphab} of \cref{alg:bvtcg} to be~$\infty$ is all points in~$\set{\sstep[k] + \alpha \pstep[k] : \alpha \ge 0}$ lie between the bounds~$\xl$ and~$\xu$.
...
\index{TCG@\glsfmtshort{tcg}!linearly-constrained|(}We now present the linearly-constrained variation of the \gls{tcg} method designed by \citeauthor{Powell_2015}~\cite{Powell_2015} for his solver \gls{lincoa}.
This algorithm is described in~\cite[\S~3]{Powell_2015}.
For ease of reference, we present it in \cref{alg:lctcg} and briefly explain the idea behind the algorithm in the sequel.
The original method in~\cite[\S~3]{Powell_2015} is devised for linear inequality constraints only, and \cref{alg:lctcg} adapts it to handle linear equality constraints as well.
...
Moreover, this tiny step would lead to a change of the working set, and as observed by \citeauthor{Powell_2015}, small steps that cause changes in the working set are numerically expensive, and hence, should be avoided~\cite[\S~3]{Powell_2015}.
...
\end{equation}
It excludes the inequality constraints with~$j \in \mathcal{J}(\sstep[0])$ and~$a_j^{\T} \pstep[0] < 0$, because any step from~$\sstep[0]$ along~$\pstep[0]$ moves further from these constraints.

After definining~$\pstep[0]$ and~$\mathcal{W}^0$, the method then considers the restriction of~\cref{eq:problem-lctcg} to the orthogonal complement of
\begin{equation*}
...
However, upon this termination, if the current iterate~$\sstep[k + 1]$ is far from the trust-region boundary, then the procedure is restarted, changing the next search direction~$\pstep[k + 1]$ to the unique solution to
\begin{subequations}
...
We solve~\cref{eq:tcg-linear-working-set,eq:tcg-linear-working-set-after} using the Goldfarb-Idnani method for quadratic programming~\cite{Goldfarb_Idnani_1983}.
...
\begin{algorithm}
    ...
    \For{$k = 0, 1, \dots$ until~$\norm{\pstep[k]_{\sstep}}^2 + \norm{\pstep[k]_v}^2 = 0$}{
        ...
        \uElseIf{$\alpha^k = \alpha_L^k$}{
            ...
            Restart the \gls{tcg} iterations by setting~$\mathcal{W}^{k + 1}$ as described in~\cref{eq:cpqp-working-set-restart}\;
        }
        \Else{
            Set~$\mathcal{W}^{k + 1} \gets \mathcal{W}^k$\;
            Set
